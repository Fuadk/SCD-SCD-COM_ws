...
const isArray = (value, callback = resultCallback, payload) => callback((typeOf(value) === 'array' || typeof value === 'object') &&
    Array.isArray(value), value, payload);

const isFunction = (value, callback = resultCallback, payload) => callback(typeof value === 'function' ||
    ...
    return {
        ...
        forEach: (forEachCallback, payload) => {
            isArray(values) &&
                ...
                values.forEach((value, index) => forEachCallback(checkFn(value), value, index, values, payload));
        },
...
const isBoolean = (value, callback = resultCallback, payload) => callback((typeof value === 'boolean' ||
    typeOf(value) === 'boolean' ||
    (typeof value === 'object' && value instanceof Boolean)) &&
    (value.valueOf() === true || value.valueOf() === false), value, payload);

const areBoolean = (...values) => areDeterminer(isBoolean, ...values);

const isDate = (value, callback = resultCallback, payload) => callback((typeOf(value) === 'date' || typeof value === 'object') &&
    value instanceof Date === true &&
    !isNaN(value), value, payload);
...
const isNumber = (value, callback = resultCallback, payload) => callback((typeof value === 'number' ||
    ((typeOf(value) === 'number' || typeof value === 'object') &&
        value instanceof Number)) &&
    !Number.isNaN(value.valueOf()) &&
    Number.isFinite(value.valueOf()), value, payload);

const areNumber = (...values) => areDeterminer(isNumber, ...values);

const isRegExp = (value, callback = resultCallback, payload) => callback((typeOf(value) === 'regexp' || typeof value === 'object') &&
    value instanceof RegExp, value, payload);

const areRegExp = (...values) => areDeterminer(isRegExp, ...values);

const isStringObject = (value, callback = resultCallback, payload) => callback((typeOf(value) === 'string' || typeof value === 'object') &&
    value instanceof String, value, payload);
...
const isClass = (value, callback = resultCallback, payload) => callback(typeof value === 'function' ||
    (typeOf(value) === 'function' && value instanceof Function)
    ? /class/.test(Function.prototype.toString.call(value).slice(0, 5))
    : false, value, payload);
...
const isObject = (value, callback = resultCallback, payload) => callback((typeof value === 'object' || typeOf(value) === 'object') &&
    value instanceof Object, value, payload);

const isInstance = (value, constructor, callback = resultCallback, payload) => callback(isObject(value) &&
    typeof constructor === 'function' &&
    constructor instanceof Function
    ? value instanceof constructor
    : false, value, { ...payload, ctor: constructor });
...
const isNumberBetween = (value, min, max, callback = resultCallback, payload) => callback(isNumber(value)
    ? (isNumberType(min) ? value.valueOf() >= min : false) &&
        (isNumberType(max) ? value.valueOf() <= max : false)
    : false, value, { ...payload, min, max });
...
const isObjectKey = (value, key, callback = resultCallback, payload) => callback(isObject(value) ? {}.hasOwnProperty.call(value, key) : false, value, { ...payload, key });
...
const isObjectKeys = (value, keys, callback = resultCallback, payload) => callback(isObject(value) && isArray(keys)
    ? keys.every((key) => ({}.hasOwnProperty.call(value, key)))
    : false, value, { ...payload, keys });

const guardObjectKeys = (value, keys, callback, payload) => isObjectKeys(value, keys, callback, payload);

const isObjectKeysIn = (value, keys, callback = resultCallback, payload) => callback(isObject(value) && isArray(keys) ? keys.every((k) => k in value) : false, value, { ...payload, keys });

const guardObjectKeysIn = (value, keys, callback, payload) => isObjectKeysIn(value, keys, callback, payload);

const isObjectSomeKeys = (value, keys, callback = resultCallback, payload) => callback(isObject(value) && isArray(keys)
    ? keys.some((someKey) => isArray(someKey)
        ? someKey.every((everyKey) => ({}.hasOwnProperty.call(value, everyKey)))
...
const isPrimitive = (value, type, callback = resultCallback, payload) => isStringType(type)
    ...
    : callback(isNull(value) ||
        (typeof value !== 'object' && typeof value !== 'function'), value, payload);
...
const isStringIncludes = (value, includes, callback = resultCallback, payload) => callback(isString(value)
    ? isArray(includes)
        ? includes.every((include) => value.valueOf().includes(include))
        : false
    : false, value, { ...payload, includes });
...
const isStringIncludesSome = (value, includes, callback = resultCallback, payload) => callback(isString(value)
    ? isArray(includes)
...
const isBooleanObject = (value, callback = resultCallback, payload) => callback((typeOf(value) === 'boolean' || typeof value === 'object') &&
    value instanceof Boolean &&
    (value.valueOf() === true || value.valueOf() === false), value, payload);

const isNumberObject = (value, callback = resultCallback, payload) => callback((typeOf(value) === 'number' || typeof value === 'object') &&
    value instanceof Number &&
    !Number.isNaN(value.valueOf()) &&
    Number.isFinite(value.valueOf()), value, payload);

const isStringLength = (value, length, callback = resultCallback, payload) => callback(isString(value) && isNumberType(length) && length > 0
    ? value.valueOf().length === length
    : false, value, { ...payload, length });

const isStringLengthBetween = (value, min, max, callback = resultCallback, payload) => callback(isString(value)
...
const isNotBoolean = (value, callback = resultCallback, payload) => callback(typeOf(value) !== 'boolean' &&
    typeof value !== 'boolean' &&
    value instanceof Boolean === false, value, payload);

const isNotDefined = (value, callback = resultCallback, payload) => callback(typeOf(value) === 'undefined' &&
    typeof value === 'undefined' &&
    value === undefined, value, payload);

const isNotFunction = (value, callback = resultCallback, payload) => callback(typeOf(value) !== 'function' &&
    typeof value !== 'function' &&
    value instanceof Function === false, value, payload);

const isNotNumber = (value, callback = resultCallback, payload) => callback(typeOf(value) !== 'number' &&
    typeof value !== 'number' &&
    value instanceof Number === false, value, payload);

const isNotString = (value, callback = resultCallback, payload) => callback(typeOf(value) !== 'string' &&
    typeof value !== 'string' &&
    value instanceof String === false, value, payload);

const isNotUndefined = (value, callback = resultCallback, payload) => callback(typeOf(value) !== 'undefined' &&
    typeof value !== 'undefined' &&
    value !== undefined, value, payload);
...
const guard = Object.freeze({ ...guardIs });
...
const recognizeValue = (value, onlyTrue = true, instances = []) => {
    ...
    RECOGNIZE_INSTANCES.concat(instances).forEach((instance) => (Object.assign(ofRecognized, { [instance.name]: value instanceof instance })));
    if (is.true(onlyTrue)) {
        Object.keys(ofRecognized).filter((type) => is.false(ofRecognized[type])
            ? delete ofRecognized[type]
            : true);
    }
    return ofRecognized;
};
