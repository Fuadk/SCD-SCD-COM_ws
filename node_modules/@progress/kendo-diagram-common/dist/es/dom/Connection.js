...
import { PathDefiner, Point, Rect } from './../math';
import { defined, isDefined, isFunction, isString, remove } from './../utils';
import { deepExtend } from './../utils/deep-extend';
import { closestConnector, filterConnectionDataItem, isAutoConnector } from './utils';
import { CascadingRouter } from './../services/routing/CascadingRouter';
import { AUTO, BOTTOM, CASCADING, CONNECTION_CONTENT_OFFSET, Cursors, INLINE, LEFT, MAXINT, NONE, RIGHT, SOURCE, TARGET, TOP, TRANSPARENT, FOCUS_CLASS, CLASS_NAME, EMPTY, CONNECTION } from './../constants';
import { Connector } from './Connector';
import { PolylineRouter } from './../services/routing/PolylineRouter';
import { ConnectionEditAdorner } from './../services/adorners/ConnectionEditAdorner';
import { CascadingConnectionResizeAdorner } from '../services/adorners/ConnectionResizeAdorner/CascadingConnection';
import { PolylineConnectionResizeAdorner } from './../services/adorners/ConnectionResizeAdorner/PolylineConnection';
import { Polyline } from './../svg';
import { ConnectionEditUnit } from './../services/ConnectionEditUnit';
import { Shape } from './Shape';
import { DiagramElement, getTextElementType } from './DiagramElement';
...
export class Connection extends DiagramElement {
    ...
    _createContentVisual(options) {
        ...
        if (hasVisual) {
            const visualResult = isFunction(template) ?
                template.call(this, templateOptions) :
                null;
            visual = visualResult;
        }
        else if (hasTemplate) {
            const templateResult = isFunction(template) ?
    ...
    _alignContent() {
        if (this._contentVisual) {
            ...
            let point;
            if (alignToPath) {
                const angle = d.util.deg(Math.atan2(endPoint.y - startPoint.y, endPoint.x - startPoint.x));
    ...
    _resolveConnectors(destination) {
        ...
        if (!sourceReferencePoint && !targetReferencePoint &&
            source instanceof Connector && target instanceof Connector) {
            const sourceConnectors = isAutoConnector(source) ? source.shape.connectors : [source];
            const targetConnectors = isAutoConnector(target) ? target.shape.connectors : [target];
