...
class DragInertia extends fx_1.Animation {
    ...
    _end() {
        ...
        /** Added by options in the constructor */
    }
    _snapBack() {
...
class ScrollBar {
    ...
    _create() {
        ...
        this.elementSize = 0;
        this.movable = new draggable_1.Movable(element);
        this.container.appendChild(element);
        this.userEvents = new user_events_1.default(element, {
            fastTap: true,
    ...
    setOptions(options) {
        ...
        if (this.options.showScrollbars) {
            if (!this.element) {
                ...
                // Update autoHideScrollbars behavior
                if (!this.autoHideScrollbars) {
                    this.show();
                    // Remove listeners if they exist, to avoid duplication or interference?
                    // If autoHideScrollbars is false, we don't need hover listeners.
                    this._toggleHoverListeners(false);
                }
                else {
                    // Add hover listeners if missing
                    this._toggleHoverListeners(true);
                    this.hide();
                }
            }
    ...
    change(e) {
        const visible = this.element.checkVisibility({ opacityProperty: true, visibilityProperty: true });
        if (!visible) {
            return;
        }
        ...
        if (range > 0 && trackRange > 0) {
            ...
            const newContentPos = Math.min(Math.max(this.startContent + contentDelta, min), max); // max is top/left (0 or +), min is bottom/right (-)
    ...
    constructor(element, options) {
        ...
        const inner = element.children[1], tapCapture = new draggable_1.TapCapture(element), movable = new draggable_1.Movable(inner), dimensions = new draggable_1.PaneDimensions({
            ...
            start: (e) => {
                ...
                if (!originatedFromFixedContainer &&
                    !avoidScrolling(e) &&
                    this.enabled &&
                    ((dimensions.x.enabled && horizontalSwipe) || (dimensions.y.enabled && verticalSwipe))) {
                    userEvents.capture();
                }
                else {
                    userEvents.cancel();
