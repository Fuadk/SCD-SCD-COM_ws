...
function documentRectToView(r, diagramRect, zoom, pan) {
    ...
    return new math_1.Rect(tl.x, tl.y, r.width * zoom, r.height * zoom);
}
...
function candidateRect(pos, anchor, tipW, tipH) {
    switch (pos) {
        case 'right':
            return new math_1.Rect(anchor.x + GAP, anchor.y - tipH / 2, tipW, tipH);
        case 'left':
            return new math_1.Rect(anchor.x - GAP - tipW, anchor.y - tipH / 2, tipW, tipH);
        case 'bottom':
            return new math_1.Rect(anchor.x - tipW / 2, anchor.y + GAP, tipW, tipH);
        case 'top':
            return new math_1.Rect(anchor.x - tipW / 2, anchor.y - GAP - tipH, tipW, tipH);
    }
}
...
function segmentBufferRect(a, b, strokeHit) {
    ...
    return new math_1.Rect(minX - strokeHit / 2, minY - strokeHit / 2, maxX - minX + strokeHit, maxY - minY + strokeHit);
}
...
function placeTooltip(options) {
    ...
    let anchorView;
    let shapeAnchors;
    if (hovered.name === constants_1.SHAPE) {
    ...
    for (const pos of positionsPriority) {
        const anchor = hovered.name === constants_1.SHAPE ? shapeAnchors[pos] : anchorView;
...
function clampRect(r, bounds) {
    ...
    return new math_1.Rect(x, y, r.width, r.height);
}
