...
const CascadingConnection_1 = require("../services/adorners/ConnectionResizeAdorner/CascadingConnection");
const PolylineConnection_1 = require("./../services/adorners/ConnectionResizeAdorner/PolylineConnection");
...
class Connection extends DiagramElement_1.DiagramElement {
    ...
    constructor(from, to, options = {}) {
        ...
        super(options);
        /** @hidden */
        this.name = constants_1.CONNECTION;
        /** @hidden */
        this.previewState = { lastSide: null };
        /** @hidden */
        ...
        if (options.locks) {
            this.locks = options.locks.map(lock => ({ ...lock }));
        }
        else {
            this.locks = [];
        }
    ...
    /** @hidden */
    _getCursor(point) {
        // Check resize adorner first (for handle hit test)
        if (this.resizeAdorner) {
            const resizeCursor = this.resizeAdorner._getCursor(point);
            if (resizeCursor !== constants_1.Cursors.select) {
                return resizeCursor;
            }
        }
        // Then check edit adorner
        if (this.adorner) {
            return this.adorner._getCursor(point);
        }
        return this.options.cursor || constants_1.Cursors.select;
    }
    /** @hidden */
        ...
        if (this._canSelect()) {
            if (this.isSelected !== value) {
                ...
                if (this.isSelected) {
                    ...
                    const ResizeAdorner = type === constants_1.CASCADING ? CascadingConnection_1.CascadingConnectionResizeAdorner : PolylineConnection_1.PolylineConnectionResizeAdorner;
    ...
    _resolveConnectors(destination) {
        ...
        // Clear resolved connectors before recalculating
        self._resolvedSourceConnector = undefined;
        self._resolvedTargetConnector = undefined;
        const source = this.source();
        const target = this.target();
        const sourceReferencePoint = target instanceof math_1.Point ? target : null;
        const targetReferencePoint = source instanceof math_1.Point ? source : null;
        // Resolve source connector
        if (source instanceof Connector_1.Connector) {
            const sourceConnectors = (0, utils_2.isAutoConnector)(source) ? source.shape.connectors : [source];
            if (sourceReferencePoint && (0, utils_2.isAutoConnector)(source)) {
                self._resolvedSourceConnector = (0, utils_2.closestConnector)(sourceReferencePoint, sourceConnectors);
            }
            else if (!(0, utils_2.isAutoConnector)(source)) {
                // For non-auto connectors, use the connector itself as the resolved connector
                self._resolvedSourceConnector = source;
            }
        }
        // Resolve target connector
        if (target instanceof Connector_1.Connector) {
            const targetConnectors = (0, utils_2.isAutoConnector)(target) ? target.shape.connectors : [target];
            if (targetReferencePoint && (0, utils_2.isAutoConnector)(target)) {
                self._resolvedTargetConnector = (0, utils_2.closestConnector)(targetReferencePoint, targetConnectors);
            }
            else if (!(0, utils_2.isAutoConnector)(target)) {
                // For non-auto connectors, use the connector itself as the resolved connector
                self._resolvedTargetConnector = target;
            }
        }
        // Resolve auto connectors when both endpoints are connectors (and no reference points)
        if (!sourceReferencePoint && !targetReferencePoint &&
            source instanceof Connector_1.Connector && target instanceof Connector_1.Connector) {
            const sourceConnectors = (0, utils_2.isAutoConnector)(source) ? source.shape.connectors : [source];
            const targetConnectors = (0, utils_2.isAutoConnector)(target) ? target.shape.connectors : [target];
    ...
    _resolveAutoConnectors(sourceConnectors, targetConnectors, destination) {
        const self = destination || this;
        // Use path-based resolution for:
        // 1. Cascading connections with manual routing (locks)
        // 2. Polyline connections with edited points
    ...
    _hasPolylineEditedPoints() {
        // For polyline connections, check if intermediate points have been added
        return this._router instanceof PolylineRouter_1.PolylineRouter && this.definers && this.definers.length > 0;
    }
    /** @hidden */
    _resolveAutoConnectorsWithLocks(sourceConnectors, targetConnectors, destination) {
        const self = destination || this;
        let minPathLength = constants_1.MAXINT;
        let minNonConflictPathLength = constants_1.MAXINT;
        let minSource, minTarget;
        let minNonConflictSource, minNonConflictTarget;
        ...
        // Prefer non-conflict route if found
        if (minNonConflictSource) {
    ...
    _calculateRouteLength(sourcePoint, targetPoint, sourceConnector, targetConnector) {
        ...
        if (router instanceof CascadingRouter_1.CascadingRouter) {
            // Get the actual route points with locks
            const points = router.routePoints(sourcePoint, targetPoint, sourceConnector, targetConnector);
            // Calculate total path length including source and target
            ...
            // For polyline with edited points, calculate path through intermediate points
        ...
        // Fallback to straight line distance if no intermediate points
    ...
    serialize() {
        ...
        if (this.locks && this.locks.length > 0) {
            json.options.locks = this.locks.map(lock => ({ ...lock }));
        }
        return json;
    }
    /**
    ...
    /** @hidden */
    focus() {
        this.path.drawingElement.options.set(constants_1.CLASS_NAME, constants_1.FOCUS_CLASS);
    }
    /** @hidden */
    blur() {
        this.path.drawingElement.options.set(constants_1.CLASS_NAME, constants_1.EMPTY);
    }
    /** @hidden */
    pointsEditable() {
