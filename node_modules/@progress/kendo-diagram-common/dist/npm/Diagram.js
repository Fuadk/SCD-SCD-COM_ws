...
const diagram_utils_1 = require("./diagram-utils");
const kendo_common_1 = require("@progress/kendo-common");
const utils_1 = require("./utils");
const constants_1 = require("./constants");
const user_events_1 = require("./utils/user-events");
const layout_1 = require("./layout");
const macOS_1 = require("./utils/macOS");
const navigation_1 = require("./navigation");
class Diagram extends Observable_1.Observable {
    /**
    ...
    constructor(element, userOptions, themeOptions) {
        ...
        this._dataMap = {};
        /** @hidden */
        this._inactiveShapeItems = new dom_1.InactiveItemsCollection();
        /** @hidden */
        this._selectedItems = [];
        ...
        this.options = (0, deep_extend_1.deepExtend)({ createToolBar: utils_1.noop, destroyToolBar: utils_1.noop }, diagram_utils_1.defaultOptions, userOptions);
        this.events = diagram_utils_1.events;
        ...
        this._initCanvas();
        this.mainLayer = new svg_1.Group({
            id: 'main-layer'
        });
        this.canvas.append(this.mainLayer);
        this._shapesQuadTree = new dom_1.ShapesQuadTree(this);
        this._pan = new math_1.Point();
        this._adorners = [];
        this.adornerLayer = new svg_1.Group({
            id: 'adorner-layer'
        });
        this.canvas.append(this.adornerLayer);
        this._createHandlers();
        this._initialize();
        this._resizingAdorner = new services_1.ResizingAdorner(this, { editable: this.options.editable });
        this._connectorsAdorner = new services_1.ConnectorsAdorner(this);
        this._adorn(this._resizingAdorner, true);
        this._adorn(this._connectorsAdorner, true);
        this.selector = new services_1.Selector(this);
        // TODO: We may consider using real Clipboard API once is supported by the standard.
        this._clipboard.length = 0;
        this.pauseMouseHandlers = false;
        this._navigation = new navigation_1.Navigation(this);
    }
    /** @hidden */
    _createShape(dataItem, options) {
        options = (0, deep_extend_1.deepExtend)({}, this.options.shapeDefaults, options);
        options.dataItem = dataItem;
        const shape = new dom_1.Shape(options, this);
        return shape;
    }
    /** @hidden */
    _createConnection(dataItem, source, target) {
        const options = (0, deep_extend_1.deepExtend)({}, this.options.connectionDefaults);
        options.dataItem = dataItem;
        const connection = new dom_1.Connection(source || new math_1.Point(), target || new math_1.Point(), options);
        return connection;
    }
    /** @hidden */
    _initElements() {
    ...
    _initDefaults(userOptions) {
        ...
        if (editable === false) {
            ...
            (0, diagram_utils_1.copyDefaultOptions)(editable, shapeDefaults.editable, ['drag', 'remove', 'connect']);
            (0, diagram_utils_1.copyDefaultOptions)(editable, connectionDefaults.editable, ['drag', 'remove']);
        }
    ...
    _interactionDefaults() {
        ...
        if (selectable && !(0, utils_1.defined)(selectable.multiple)) {
            options.selectable = (0, deep_extend_1.deepExtend)({
                multiple: mobile ? false : true
            }, options.selectable);
        }
        if (pannable === true) {
            options.pannable = { ...diagram_utils_1.defaultOptions.pannable };
        }
        if (pannable && !(0, utils_1.defined)(pannable.key)) {
            options.pannable = (0, deep_extend_1.deepExtend)({
    ...
    _initCanvas() {
        ...
        this.canvas = new (this.options.Canvas || svg_1.Canvas)(canvasContainer, {
            width: viewPort.width || constants_1.DEFAULT_CANVAS_WIDTH,
            height: viewPort.height || constants_1.DEFAULT_CANVAS_HEIGHT
        });
    }
    /** @hidden */
    _createHandlers() {
        ...
        this._userEvents = new user_events_1.default(this.scrollable, {
            multiTouch: true,
            fastTap: true,
        ...
        this.toolService = new services_1.ToolService(this);
        ...
        diagram_utils_1.domEvents.forEach(event => {
            this.scrollable.addEventListener(event, this._domEvent);
        });
        this._initResizeObserver();
        this.bind(constants_1.ZOOM_START, this._destroyToolBar.bind(this));
        this.bind(constants_1.PAN, this._destroyToolBar.bind(this));
        this.bind(constants_1.MOUSE_ENTER, this._onMouseEnter.bind(this));
        this.bind(constants_1.MOUSE_LEAVE, this._onMouseLeave.bind(this));
    }
    _onMouseEnter(event) {
        const { delay } = this.options.tooltip;
        this._tooltipTimeOut = setTimeout(() => {
            this.trigger(constants_1.TOOLTIP_SHOW, event);
            this._tooltipTimeOut = null;
        }, delay);
    }
    _onMouseLeave(event) {
        clearTimeout(this._tooltipTimeOut);
        this.trigger(constants_1.TOOLTIP_HIDE, event);
    }
    /** @hidden */
    _initResizeObserver() {
    ...
    _mouseDown(e) {
        ...
        if (e.button === 1 && this.options.pannable !== false && !this.toolService.hoveredItem) {
            ...
            const meta = (0, deep_extend_1.deepExtend)(this._meta(e), { middleButton: true });
    ...
    _tap(e) {
        ...
        if (toolService.hoveredItem) {
            ...
            if (selectable && item.options.selectable !== false) {
                ...
                const ctrlPressed = kendo_common_1.mobileOS || meta.ctrlKey || (meta.metaKey && (0, macOS_1.macOS)());
                if (item.isSelected) {
                    if (ctrlPressed) {
                        this._destroyToolBar();
                        item.select(false);
    ...
    /** @hidden */
    _wheel(e) {
        const delta = (0, dom_1.mwDelta)(e), p = this._eventPositions(e), meta = (0, deep_extend_1.deepExtend)(this._meta(e), { delta: delta });
    ...
    _eventPositions(e, start) {
        ...
        if (e.touch) {
            ...
            point = new math_1.Point(e.x[field], e.y[field]);
        }
        else {
            point = new math_1.Point(e.pageX, e.pageY);
        }
    ...
    _gestureStart(e) {
        ...
        const initialCenter = this.documentToModel(new math_1.Point(e.center.x, e.center.y));
        const eventArgs = {
            point: initialCenter,
            zoom: this.zoom()
        };
        if (this.trigger(constants_1.ZOOM_START, eventArgs)) {
            return;
        }
        this._gesture = e;
    ...
    _gestureChange(e) {
        ...
        const center = this.documentToView(new math_1.Point(e.center.x, e.center.y));
        const scaleDelta = e.distance / previousGesture.distance;
        let zoom = this._zoom;
        let updateZoom = false;
        if (Math.abs(scaleDelta - 1) >= constants_1.MOBILE_ZOOM_RATE) {
        ...
        if (updateZoom || this._pan.distanceTo(pan) >= constants_1.MOBILE_PAN_DISTANCE) {
    ...
    _doubleTap(e) {
        ...
        if (this.trigger(constants_1.ZOOM_START, zoomOptions)) {
            return;
        }
        zoom = kendo_drawing_1.drawing.util.round(Math.max(options.zoomMin, Math.min(options.zoomMax, zoom)), 2);
        zoomOptions.zoom = zoom;
        this.zoom(zoom, zoomOptions);
        this.trigger(constants_1.ZOOM_END, zoomOptions);
    }
    /** @hidden */
    _gestureEnd() {
        ...
        this.trigger(constants_1.ZOOM_END, {
    ...
    _initTheme(themeOptions) {
        ...
        this.options = (0, deep_extend_1.deepExtend)({}, themeOptions, this.options);
        if (this.options.editable === true) {
            this.options.editable = (themeOptions || {}).editable;
        }
        else if (this.options.editable && this.options.editable.resize === true) {
            // If user passed resize: true, use theme's resize options (they will be merged with defaults in ResizingAdorner)
    ...
    _createConnections() {
        ...
        for (i = 0; i < connections.length; i++) {
            ...
            if (this.options.connect) {
                this.options.connect(source, target, (0, deep_extend_1.deepExtend)({}, defaults, conn));
            }
            else {
                this.connect(source, target, (0, deep_extend_1.deepExtend)({}, defaults, conn));
            }
        }
    }
    /** @hidden */
    _findConnectionTarget(options) {
        options = options || {};
        const shapeId = (0, utils_1.isString)(options) ? options : options.shapeId || options.id;
        let target;
        if (shapeId) {
            ...
            target = new math_1.Point(options.x || 0, options.y || 0);
        }
        return target;
    }
    /**
    ...
    save() {
        ...
        for (i = 0; i < this.shapes.length; i++) {
            ...
            if (shape.options.serializable) {
                const shapeOptions = (0, deep_extend_1.deepExtend)({}, shape.options);
        ...
        for (i = 0; i < this.connections.length; i++) {
            ...
            json.connections.push((0, deep_extend_1.deepExtend)({}, connection.options, connection.toJSON()));
        }
        return json;
    }
    /**
     ...
     */
    setOptions(options) {
        (0, deep_extend_1.deepExtend)(this.options, options);
        if (options.pannable) {
    ...
    /** @hidden */
    enterEdit(item) {
        if ((item === null || item === void 0 ? void 0 : item.name) === constants_1.SHAPE && this._connectorsAdorner) {
    ...
    disablePan() {
        ...
        this._currentPannable = typeof pannable === constants_1.OBJECT ? { ...pannable } : pannable;
        this.setOptions({ pannable: { showScrollbars: false, key: 'none' } });
    }
    /** @hidden */
    restorePan() {
        this.setOptions({ pannable: this._currentPannable });
        this._currentPannable = undefined;
    }
    /**
    ...
    addConnection(connection, undoable) {
        if (undoable !== false) {
            this.undoRedoService.add(new services_1.AddConnectionUnit(connection, this), false);
        }
        ...
        this.trigger(constants_1.CHANGE, {
            added: [connection],
            removed: []
        });
        return connection;
    ...
    addShape(item, undoable) {
        ...
        if (item instanceof dom_1.Shape) {
            shape = item;
            this._parseBounds(shape.bounds());
        }
        else if (!(item.prototype)) {
            shapeDefaults = (0, deep_extend_1.deepExtend)({}, shapeDefaults, item || {});
            shape = new dom_1.Shape(shapeDefaults, this);
            this._parseBounds(shape.bounds());
        }
        else {
            return;
        ...
        if (undoable !== false) {
            this.undoRedoService.add(new services_1.AddShapeUnit(shape, this), false);
        }
    ...
    remove(items, undoable) {
        ...
        const elements = (0, diagram_utils_1.splitDiagramElements)(items);
        const shapes = elements.shapes;
        const connections = elements.connections;
        let i;
        if (!(0, utils_1.defined)(undoable)) {
            undoable = true;
        }
        if (undoable) {
    ...
    _addConnectionDataItem(dataItem, undoable) {
        if (!this._connectionsDataMap[dataItem.uid]) {
            ...
            if (!(0, utils_1.defined)(from) || from === null) {
                from = new math_1.Point(dataItem.fromX, dataItem.fromY);
            }
            let to = this._validateConnector(dataItem.to);
            if (!(0, utils_1.defined)(to) || to === null) {
                to = new math_1.Point(dataItem.toX, dataItem.toY);
            }
            if ((0, utils_1.defined)(from) && (0, utils_1.defined)(to)) {
                const options = (0, deep_extend_1.deepExtend)({}, this.options.connectionDefaults);
                options.dataItem = dataItem;
                const connection = new dom_1.Connection(from, to, options);
    ...
    _validateConnector(value) {
        let connector;
        if ((0, utils_1.defined)(value) && value !== null) {
    ...
    connect(source, target, options) {
        const resolvedOptions = (0, deep_extend_1.deepExtend)({}, this.options.connectionDefaults, options);
        const connection = new dom_1.Connection(source, target, resolvedOptions);
     ...
     */
    select(item, options) {
        if ((0, utils_1.isDefined)(item)) {
            options = (0, deep_extend_1.deepExtend)({ addToSelection: false }, options);
            ...
            else if (item instanceof dom_1.DiagramElement) {
                items = [item];
            }
    ...
    selectArea(rect) {
        ...
        this._internalSelection = true;
        const selected = [];
        if (rect instanceof math_1.Rect) {
    ...
    deselect(item) {
        ...
        else if (!(0, utils_1.isDefined)(item)) {
    ...
    toFront(items, undoable) {
        ...
        let indices;
        if (!(0, utils_1.defined)(undoable) || undoable) {
            indices = (0, dom_1.indicesOfItems)(this.mainLayer, result.visuals);
            const unit = new services_1.ToFrontUnit(this, items, indices);
    ...
    toBack(items, undoable) {
        ...
        if (!(0, utils_1.defined)(undoable) || undoable) {
            indices = (0, dom_1.indicesOfItems)(this.mainLayer, result.visuals);
            const unit = new services_1.ToBackUnit(this, items, indices);
            this.undoRedoService.add(unit);
        }
        else {
    ...
    bringIntoView(item, options) {
        const viewport = this.viewport();
        const aligner = new math_1.RectAlign(viewport);
        let rect;
        if (viewport.width === 0 || viewport.height === 0) {
            return;
        }
        options = (0, deep_extend_1.deepExtend)({ animate: false, align: 'center middle' }, options);
        if (options.align === 'none') {
            options.align = 'center middle';
        }
        if (item instanceof dom_1.DiagramElement) {
            rect = item.bounds(constants_1.TRANSFORMED);
        }
        else if (Array.isArray(item)) {
            rect = this.boundingBox(item);
        }
        else if (item instanceof math_1.Rect) {
            rect = item.clone();
        }
    ...
    alignShapes(direction) {
        if ((0, utils_1.isUndefined)(direction)) {
            direction = 'Left';
        }
        let val, item, i;
        ...
        switch (direction.toLowerCase()) {
            case 'left':
            case 'top':
                val = constants_1.MAX_VALUE;
                break;
            case 'right':
            case 'bottom':
                val = constants_1.MIN_VALUE;
                break;
            default:
                break;
        }
        ...
        for (i = 0; i < items.length; i++) {
            ...
            if (item instanceof dom_1.Shape) {
                ...
                switch (direction.toLowerCase()) {
                    case 'left':
                    case 'right':
                        item.position(new math_1.Point(val, item.options.y));
                        break;
                    case 'top':
                    case 'bottom':
                        item.position(new math_1.Point(item.options.x, val));
                        break;
                    default:
                        break;
                }
        ...
        const unit = new services_1.TransformUnit(shapes, undoStates);
    ...
    zoom(zoom, options) {
        if (zoom) {
            let staticPoint = options ? options.point : new math_1.Point(0, 0);
            // var meta = options ? options.meta : 0;
            zoom = this._zoom = this._getValidZoom(zoom);
            if (!(0, utils_1.isUndefined)(staticPoint)) { // Viewpoint vector is constant
                staticPoint = new math_1.Point(Math.round(staticPoint.x), Math.round(staticPoint.y));
                ...
                this._storePan(new math_1.Point(Math.round(raw.x), Math.round(raw.y)));
            }
            if (options) {
                options.zoom = zoom;
            }
     ...
     */
    pan(pan, animate) {
        if (pan instanceof math_1.Point) {
            const scroller = this.scroller;
    ...
    viewport() {
        const element = this.element;
        const width = (0, diagram_utils_1.elementWidth)(element);
        let height = (0, diagram_utils_1.elementHeight)(element);
        if (this.toolBar) {
            height -= (0, diagram_utils_1.outerHeight)(this.toolBar.element);
        }
        return new math_1.Rect(0, 0, width, height);
    }
    /**
    ...
    paste() {
        if (this._clipboard.length > 0) {
            let item, copied, i;
            const mapping = {};
            const elements = (0, diagram_utils_1.splitDiagramElements)(this._clipboard);
            const connections = elements.connections;
            const shapes = elements.shapes;
            const offset = {
            ...
            for (i = 0; i < shapes.length; i++) {
                ...
                copied.position(new math_1.Point(item.options.x + offset.x, item.options.y + offset.y));
                copied.diagram = this;
    ...
    _updateCopiedConnection(connection, sourceConnection, connectorName, mapping, offset) {
        ...
        if (target instanceof dom_1.Connector && mapping[target.shape.id]) {
            ...
            connection[connectorName](new math_1.Point(sourceConnection[connectorName + 'Point']().x + offset.x, sourceConnection[connectorName + 'Point']().y + offset.y));
        }
    }
    /**
    ...
    boundingBox(items, origin) {
        let rect = math_1.Rect.empty(), temp;
        const di = (0, utils_1.isDefined)(items) ? this._getDiagramItems(items) : { shapes: this.shapes };
        if (di.shapes.length > 0) {
            let item = di.shapes[0];
            rect = item.bounds(constants_1.ROTATED);
            for (let i = 1; i < di.shapes.length; i++) {
                item = di.shapes[i];
                temp = item.bounds(constants_1.ROTATED);
                if (origin === true) {
    ...
    /** @hidden */
    _containerOffset() {
        const containerOffset = (0, diagram_utils_1.elementOffset)(this.element);
        if (this.toolBar) {
            containerOffset.top += (0, diagram_utils_1.outerHeight)(this.toolBar.element);
        }
    ...
    documentToView(point) {
        ...
        return new math_1.Point(point.x - containerOffset.left, point.y - containerOffset.top);
    }
    /**
    ...
    _transformWithMatrix(point, matrix) {
        let result = point;
        if (point instanceof math_1.Point) {
            if (matrix) {
            ...
            result = math_1.Rect.fromPoints(tl, br);
        }
        return result;
    }
    /**
    ...
    layout(options) {
        ...
        let type;
        if ((0, utils_1.isUndefined)(options)) {
            options = this.options.layout;
        }
        if ((0, utils_1.isUndefined)(options) || (0, utils_1.isUndefined)(options.type)) {
            type = 'Tree';
        }
        else {
            type = options.type;
        ...
        switch (type.toLowerCase()) {
            case 'tree':
                l = new layout_1.TreeLayout(this);
                break;
            case 'layered':
                l = new layout_1.LayeredLayout(this);
                break;
            case 'forcedirected':
            case 'force':
            case 'spring':
            case 'springembedder':
                l = new math_1.SpringLayout(this);
                break;
            default:
                throw new Error('Layout algorithm \'' + type + '\' is not supported.');
        }
        const initialState = new layout_1.LayoutState(this);
        const finalState = l.layout(options);
        if (finalState) {
            const unit = new services_1.LayoutUndoUnit(initialState, finalState, options ? options.animate : null);
            this.undoRedoService.add(unit);
        }
    ...
    getShapeById(id) {
        let found;
        found = (0, utils_1.first)(this.shapes, function (s) {
            return s.visual.id === id;
        });
        if (found) {
            return found;
        }
        found = (0, utils_1.first)(this.connections, function (c) {
    ...
    getShapeByModelId(id) {
        ...
        if (this._isEditable) {
            ...
            shapeResult = (0, utils_1.first)(this.shapes, function (shape) {
    ...
    _extendLayoutOptions(options) {
        if (options.layout) {
            options.layout = (0, deep_extend_1.deepExtend)({}, layout_1.LayoutDefaultOptions, options.layout);
        }
    }
    /** @hidden */
    _selectionChanged(selected, deselected) {
        if (selected.length || deselected.length) {
            this.trigger(constants_1.SELECT, { selected: selected, deselected: deselected });
        }
    }
    /** @hidden */
    _getValidZoom(zoom) {
    ...
    /** @hidden */
    _finishPan() {
        this.trigger(constants_1.PAN, { total: this._pan, delta: Number.NaN });
    }
    /** @hidden */
    _storePan(pan) {
        this._pan = pan;
    ...
    _zoomMainLayer() {
        ...
        const transform = new svg_1.CompositeTransform(0, 0, zoom, zoom);
    ...
    _transformMainLayer() {
        ...
        const transform = new svg_1.CompositeTransform(pan.x, pan.y, zoom, zoom);
        transform.render(this.mainLayer);
        this._storeLayerMatrix(transform);
        this._storeViewMatrix();
    }
    ...
    _fixOrdering(result, toFront) {
        ...
        for (i = 0; i < result.shapes.length; i++) {
            ...
            (0, utils_1.remove)(this.shapes, item);
            (0, utils_1.insert)(this.shapes, item, shapePos);
        }
        for (i = 0; i < result.cons.length; i++) {
            item = result.cons[i];
            (0, utils_1.remove)(this.connections, item);
            (0, utils_1.insert)(this.connections, item, conPos);
        }
    }
    /** @hidden */
    _getDiagramItems(items) {
        ...
        for (i = 0; i < args.length; i++) {
            ...
            else if (item instanceof dom_1.Connection) {
    ...
    _addDataItemByUid(dataItem) {
        ...
        const options = (0, deep_extend_1.deepExtend)({}, this.options.shapeDefaults);
        options.dataItem = dataItem;
        shape = new dom_1.Shape(options, this);
        this.addShape(shape);
    ...
    _initialize() {
        ...
        this.undoRedoService = new services_1.UndoRedoService({
    ...
    _adorn(adorner, isActive) {
        if (isActive !== undefined && adorner) {
            if (isActive) {
                ...
                (0, utils_1.remove)(this._adorners, adorner);
    ...
    _removeShape(shape, undoable, removedConnections) {
        ...
        if (undoable) {
            this.undoRedoService.addCompositeItem(new services_1.DeleteShapeUnit(shape));
        }
        (0, utils_1.remove)(this.shapes, shape);
        this._shapesQuadTree.remove(shape);
        for (i = 0; i < shape.connectors.length; i++) {
            connector = shape.connectors[i];
            for (let j = 0; j < connector.connections.length; j++) {
                connection = connector.connections[j];
                if (!removedConnections || !(0, utils_1.contains)(removedConnections, connection)) {
    ...
    _removeConnection(connection, undoable) {
        if (connection.sourceConnector) {
            (0, utils_1.remove)(connection.sourceConnector.connections, connection);
        }
        if (connection.targetConnector) {
            (0, utils_1.remove)(connection.targetConnector.connections, connection);
        }
        if (undoable) {
            this.undoRedoService.addCompositeItem(new services_1.DeleteConnectionUnit(connection));
        }
        (0, utils_1.remove)(this.connections, connection);
    }
    /** @hidden */
    ...
    /** @hidden */
    _mobileOS() {
        return kendo_common_1.mobileOS;
    }
    /**
    ...
    exportDOMVisual() {
        ...
        const scrollOffset = kendo_drawing_1.geometry.transform()
            .translate(-viewBox.x, -viewBox.y);
        const viewRect = new kendo_drawing_1.geometry.Rect([0, 0], [viewBox.width, viewBox.height]);
        const clipPath = kendo_drawing_1.drawing.Path.fromRect(viewRect);
        const wrap = new kendo_drawing_1.drawing.Group({ transform: scrollOffset });
        const clipWrap = new kendo_drawing_1.drawing.Group({ clip: clipPath });
    ...
    exportVisual() {
        ...
        const scale = kendo_drawing_1.geometry.transform().scale(scaleX, scaleX);
        const wrap = new kendo_drawing_1.drawing.Group({
            transform: scale
        });
