...
const ConnectionEditTool_1 = require("./ConnectionEditTool");
const ConnectionResizeTool_1 = require("./ConnectionResizeTool");
const ConnectionTool_1 = require("./ConnectionTool");
const PointerTool_1 = require("./PointerTool");
const ScrollerTool_1 = require("./ScrollerTool");
const SelectionTool_1 = require("./SelectionTool");
const testKey_1 = require("./testKey");
const Connection_1 = require("./../../dom/Connection");
const math_1 = require("./../../math");
const utils_1 = require("./../../utils");
const round = kendo_drawing_1.drawing.util.round;
function hitTestShapeConnectors(shape, point) {
    let connector, position, rect;
    for (let idx = 0; idx < shape.connectors.length; idx++) {
        connector = shape.connectors[idx];
        position = connector.position();
        rect = new math_1.Rect(position.x, position.y);
        rect.inflate(constants_1.HIT_TEST_DISTANCE, constants_1.HIT_TEST_DISTANCE);
...
class ToolService {
    ...
    constructor(diagram) {
        ...
        this.tools = [
            new ScrollerTool_1.ScrollerTool(this),
            new ConnectionResizeTool_1.ConnectionResizeTool(this),
            new ConnectionEditTool_1.ConnectionEditTool(this),
            new ConnectionTool_1.ConnectionTool(this),
            new SelectionTool_1.SelectionTool(this),
            new PointerTool_1.PointerTool(this)
        ]; // the order matters.
    ...
    start(p, meta, nativeEvent) {
        meta = (0, deep_extend_1.deepExtend)({}, meta);
        if (this.activeTool) {
    ...
    keyDown(key, meta) {
        ...
        meta = (0, deep_extend_1.deepExtend)({ ctrlKey: false, metaKey: false, altKey: false }, meta);
        if ((meta.ctrlKey || meta.metaKey) && !meta.altKey) { // ctrl or option
            if ((0, testKey_1.testKey)(key, 'a')) { // A: select all
                diagram.selectAll();
                diagram._destroyToolBar();
                return true;
            }
            else if ((0, testKey_1.testKey)(key, 'z')) { // Z: undo
                diagram.undo();
                diagram._destroyToolBar();
                return true;
            }
            else if ((0, testKey_1.testKey)(key, 'y')) { // y: redo
                diagram.redo();
                diagram._destroyToolBar();
                return true;
            }
            else if ((0, testKey_1.testKey)(key, 'c')) {
                diagram.copy();
                diagram._destroyToolBar();
            }
            else if ((0, testKey_1.testKey)(key, 'x')) {
                diagram.cut();
                diagram._destroyToolBar();
            }
            else if ((0, testKey_1.testKey)(key, 'v')) {
                diagram.paste();
                diagram._destroyToolBar();
            }
            else if ((0, testKey_1.testKey)(key, 'l')) {
                diagram.layout();
                diagram._destroyToolBar();
            }
            else if ((0, testKey_1.testKey)(key, 'd')) {
                diagram._destroyToolBar();
                diagram.copy();
                diagram.paste();
            }
    ...
    wheel(p, meta, nativeEvent) {
        ...
        if (diagram.trigger(constants_1.ZOOM_START, zoomOptions)) {
            return;
        }
        if (delta < 0) {
            z += zoomRate;
        ...
        diagram.trigger(constants_1.ZOOM_END, zoomOptions);
        return true;
    }
    /**
    ...
    _updateCursor(p) {
        ...
        if (this.activeTool) {
            ...
            // If adorner returns undefined, fall back to hovered item
        ...
        element.style.cursor = cursor || constants_1.Cursors.arrow;
    }
    /** @hidden */
    ...
    _updateHoveredItem(point, meta, nativeEvent) {
        ...
        if (hit !== this.hoveredItem && (!this.disabledShape || hit !== this.disabledShape)) {
            if (this.hoveredItem) {
                diagram.trigger(constants_1.MOUSE_LEAVE, { item: this.hoveredItem, nativeEvent, point, meta });
                this.hoveredItem._hover(false);
            }
            if (hit && hit.options.enable) {
                diagram.trigger(constants_1.MOUSE_ENTER, { item: hit, nativeEvent, point, meta });
    ...
    _hitTest(point) {
        ...
        if (!this.activeTool || this.activeTool.type !== 'ConnectionTool') {
            ...
            for (i = 0; i < d._selectedItems.length; i++) {
                ...
                if (item instanceof Connection_1.Connection) {
    ...
    _hitTestElements(point) {
        ...
        if ((!this.activeTool || this.activeTool.type !== 'ConnectionTool') && shapeHit && connectionHit && !hitTestShapeConnectors(shapeHit, point)) {
            ...
            const shapeIdx = (0, utils_1.inArray)(shapeHit.visual, mainLayer.children);
            const connectionIdx = (0, utils_1.inArray)(connectionHit.visual, mainLayer.children);
