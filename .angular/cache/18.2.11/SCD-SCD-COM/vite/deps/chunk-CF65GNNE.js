...
function BpmnRenderer(config, eventBus, styles, pathMap, canvas, textRenderer, priority) {
  ...
  function createMarker(parentGfx, id, type, fill, stroke) {
    if (type === "sequenceflow-end") {
      var sequenceflowEnd = create("path", __spreadValues({
    ...
    if (type === "messageflow-start") {
      var messageflowStart = create("circle", __spreadValues({
        cx: 6,
        cy: 6,
        r: 3.5
      }, shapeStyle({
    ...
    if (type === "messageflow-end") {
      var messageflowEnd = create("path", __spreadValues({
    ...
    if (type === "association-start") {
      var associationStart = create("path", __spreadValues({
    ...
    if (type === "association-end") {
      var associationEnd = create("path", __spreadValues({
    ...
    if (type === "conditional-flow-marker") {
      var conditionalFlowMarker = create("path", __spreadValues({
    ...
    if (type === "conditional-default-flow-marker") {
      var defaultFlowMarker = create("path", __spreadValues({
        d: "M 6 4 L 10 16"
      }, shapeStyle({
        stroke,
        fill: "none"
  ...
  function drawCircle(parentGfx, width, height, offset, attrs = {}) {
    ...
    var circle = create("circle", __spreadValues({
      cx,
      cy,
  ...
  function drawRect(parentGfx, width, height, r, offset, attrs) {
    ...
    offset = offset || 0;
    attrs = shapeStyle(attrs);
    var rect = create("rect", __spreadValues({
      x: offset,
      y: offset,
  ...
  function drawDiamond(parentGfx, width, height, attrs) {
    ...
    attrs = shapeStyle(attrs);
    var polygon = create("polygon", __spreadProps(__spreadValues({}, attrs), {
      points: pointsString
    }));
  ...
  function drawPath2(parentGfx, d, attrs) {
    attrs = lineStyle(attrs);
    var path = create("path", __spreadProps(__spreadValues({}, attrs), {
      d
    }));
  ...
  function drawMarker(type, parentGfx, path, attrs) {
    return drawPath2(parentGfx, path, assign({
      "data-marker": type
    }, attrs));
  }
  ...
  function renderActivity(parentGfx, element, attrs = {}) {
    ...
    return drawRect(parentGfx, width, height, TASK_BORDER_RADIUS, __spreadProps(__spreadValues({}, attrs), {
      fill: getFillColor(element, defaultFillColor, attrs.fill),
  ...
  function renderAssociation(parentGfx, element, attrs = {}) {
    ...
    return drawConnectionSegments(parentGfx, element.waypoints, __spreadProps(__spreadValues({}, attrs), {
      stroke,
      strokeDasharray: "0, 5"
    }));
  }
  ...
  function renderEvent(parentGfx, element, attrs = {}) {
    return drawCircle(parentGfx, element.width, element.height, __spreadProps(__spreadValues({
      fillOpacity: DEFAULT_OPACITY
    }, attrs), {
      fill: getFillColor(element, defaultFillColor, attrs.fill),
      stroke: getStrokeColor(element, defaultStrokeColor, attrs.stroke)
  ...
  var handlers = this.handlers = {
    ...
    "bpmn:BoundaryEvent": function(parentGfx, element, attrs = {}) {
      ...
      drawCircle(parentGfx, element.width, element.height, INNER_OUTER_DIST, __spreadProps(__spreadValues({}, attrs), {
        fill: "none"
      }));
      if (renderIcon) {
    ...
    "bpmn:CallActivity": function(parentGfx, element, attrs = {}) {
      attrs = pickAttrs(attrs, ["fill", "stroke"]);
      return renderSubProcess(parentGfx, element, __spreadValues({
        strokeWidth: 5
      }, attrs));
    },
    ...
    "bpmn:DataInputAssociation": function(parentGfx, element, attrs = {}) {
      attrs = pickAttrs(attrs, ["fill", "stroke"]);
      return renderAssociation(parentGfx, element, __spreadProps(__spreadValues({}, attrs), {
    ...
    "bpmn:EndEvent": function(parentGfx, element, attrs = {}) {
      ...
      attrs = pickAttrs(attrs, ["fill", "stroke"]);
      var event2 = renderEvent(parentGfx, element, __spreadProps(__spreadValues({}, attrs), {
        strokeWidth: 4
      }));
      if (renderIcon) {
        renderEventIcon(element, parentGfx, attrs);
    ...
    "bpmn:IntermediateEvent": function(parentGfx, element, attrs = {}) {
      ...
      attrs = pickAttrs(attrs, ["fill", "stroke"]);
      var outer = renderEvent(parentGfx, element, __spreadProps(__spreadValues({}, attrs), {
        strokeWidth: 1.5
      }));
    ...
    "bpmn:Lane": function(parentGfx, element, attrs = {}) {
      attrs = pickAttrs(attrs, ["fill", "stroke", "width", "height"]);
      return renderLane(parentGfx, element, __spreadProps(__spreadValues({}, attrs), {
    ...
    "bpmn:Transaction": function(parentGfx, element, attrs = {}) {
      ...
      var outer = renderSubProcess(parentGfx, element, __spreadValues({
        strokeWidth: 1.5
      }, attrs));
...
var core_default = {
  __depends__: [draw_default, import_default]
};
...
function getClosure(elements, isTopLevel, closure) {
  ...
  var topLevel = copyObject(closure.topLevel, isTopLevel && groupBy(elements, function(e2) {
    return e2.id;
  }));
...
function DrilldownOverlayBehavior(canvas, eventBus, elementRegistry, overlays, translate3) {
  ...
  this.executed(["shape.create", "shape.move", "shape.delete"], LOW_PRIORITY2, function(context) {
  ...
  this.reverted(["shape.create", "shape.move", "shape.delete"], LOW_PRIORITY2, function(context) {
    var oldParent = context.oldParent, newParent = context.newParent || context.parent, shape = context.shape;
    if (self._canDrillDown(shape)) {
      self._addOverlay(shape);
    }
...
DrilldownOverlayBehavior.prototype._addOverlay = function(element) {
  ...
  var button = domify$1('<button type="button" class="bjs-drilldown">' + ARROW_DOWN_SVG + "</button>"), elementName = bo.get("name") || bo.get("id"), title = this._translate("Open {element}", {
    element: elementName
  });
...
function createPath(path, attrs, OUTLINE_STYLE) {
  return create("path", __spreadValues({
    d: path,
    strokeWidth: 2,
...
function InteractionEvents(eventBus, elementRegistry, styles) {
  ...
  eventBus.on(["shape.changed", "connection.changed"], LOW_PRIORITY4, function(event2) {
    var element = event2.element, gfx = event2.gfx;
...
ElementSerializer.prototype.parseGenericAttributes = function(element, attributes) {
  ...
  forEach(attributes, function(attr3) {
    ...
    try {
      ...
      console.warn("missing namespace information for ", attr3.name, "=", attr3.value, "on", element, e2);
    }
  });
};
...
BaseViewer.prototype.importXML = function importXML(xml2, bpmnDiagram) {
  return __async(this, null, function* () {
    ...
    try {
      xml2 = this._emit("import.parse.start", {
        xml: xml2
      }) || xml2;
      let parseResult;
      try {
        parseResult = yield this._moddle.fromXML(xml2, "bpmn:Definitions");
      } catch (error3) {
      ...
      const importResult = yield this.importDefinitions(definitions, bpmnDiagram);
...
BaseViewer.prototype.importDefinitions = function importDefinitions(definitions, bpmnDiagram) {
  return __async(this, null, function* () {
    this._setDefinitions(definitions);
    const result = yield this.open(bpmnDiagram);
    return {
      warnings: result.warnings
    };
  });
};
BaseViewer.prototype.open = function open2(bpmnDiagramOrId) {
  return __async(this, null, function* () {
    ...
    const {
      warnings
    } = yield importBpmnDiagram(this, definitions, bpmnDiagram);
    return {
      warnings
    };
  });
};
BaseViewer.prototype.saveXML = function saveXML(options) {
  return __async(this, null, function* () {
    options = options || {};
    ...
    try {
      ...
      const result2 = yield this._moddle.toXML(definitions, options);
      xml2 = result2.xml;
...
BaseViewer.prototype.saveSVG = function saveSVG() {
  return __async(this, null, function* () {
...
e(Viewer, BaseViewer);
Viewer.prototype._modules = [core_default, drilldown_default, outline_default2, overlays_default, selection_default, translate_default];
...
//# sourceMappingURL=chunk-CF65GNNE.js.map
