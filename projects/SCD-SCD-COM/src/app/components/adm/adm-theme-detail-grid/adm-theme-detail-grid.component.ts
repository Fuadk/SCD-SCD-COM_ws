...
import {   themedetailMysecondappThemeDetailGrid , componentConfigDef } from '@modeldir/model';
...
'THEME_VARIABLE' : new FormControl(dataItem.THEME_VARIABLE  , Validators.required) ,
'THEME_VARIABLE_VALUE' : new FormControl(dataItem.THEME_VARIABLE_VALUE  , )
});
...
@Component({
  selector: 'app-adm-theme-detail-grid',
  encapsulation: ViewEncapsulation.None,
  templateUrl: './adm-theme-detail-grid.component.html',
  styleUrls: ['./adm-theme-detail-grid.component.scss'
],
...
export class AdmThemeDetailGridGridComponent implements OnInit,OnDestroy {
 ...
 public showToolBar = true;
  ...
  private masterKey ="";
  private masterKeyName ="THEME_NAME";
  private insertCMD = "INSERT_THEME_DETAIL";
  private updateCMD = "UPDATE_THEME_DETAIL";
  private deleteCMD =   "DELETE_THEME_DETAIL";
  private getCMD = "GET_THEME_DETAIL_QUERY";

  public  executeQueryresult:any;
  public title =  this.starServices.getNLS([],"MySecondApp_THEME_DETAIL_GRID.themedetailMysecondappThemeDetailGrid.component_title","Theme Detail");
  ...
  public primarKeyReadOnlyArr = {isTHEME_NAMEreadOnly : false, isTHEME_VARIABLEreadOnly : false};
  public paramConfig;
...
public compSelector = 'app-adm-theme-detail-grid';
...
//Next part for filtering
  ...
  @Input() public set detail_Input(grid: any) {
    if ( (grid.THEME_NAME != "") &&   (typeof grid.THEME_NAME != "undefined"))
    {
      this.masterKey = grid.THEME_NAME;
  ...
  private gridInitialValues = new themedetailMysecondappThemeDetailGrid();
 ...
 async cellClickHandler({ isEdited, dataItem, rowIndex }) {
    ...
    await this.ON_CLICK(this.formGroup.value, rowIndex);
    this.readCompletedOutput.emit(this.formGroup.value);
     ...
     let componentConfig = new componentConfigDef();
      ...
      let masterKeyArr = [this.formGroup.value['THEME_NAME']];
  ...
  public enterQuery (grid : GridComponent): void{
   ...
   async callBackFunction(data) {
      ...
      let masterKeyArr = [GridData.data[0]['THEME_NAME']];
      let masterKeyNameArr = ['THEME_NAME'];
      //for (let i = 0; i < masterKeyNameArr.length; i++) {
      //  componentConfig.[masterKeyNameArr[i]] = masterKeyArr[i];
      //}
...
public saveCurrent() {
  ...
  this.lookupArrDef =[	{"statment":"SELECT CODE, CODETEXT_LANG, CODEVALUE_LANG FROM SOM_TABS_CODES WHERE CODENAME ='THEME_VARIABLE' and LANGUAGE_NAME = '" + this.userLang + "' order by CODETEXT_LANG",
        "lkpArrName":"lkpArrTHEME_VARIABLE"}];
  this.starServices.fetchLookups(this, this.lookupArrDef);
  }
  
  public lkpArrTHEME_VARIABLE = [];
  
  public lkpArrGetTHEME_VARIABLE(CODE: any): any {
  var rec = this.lkpArrTHEME_VARIABLE.find((x:any) => x.CODE === CODE);
  return rec;
  }
  ...
  window.print();
   ...
   public handleComponentConfig(ComponentConfig) {
      ...
      if (typeof ComponentConfig !== "undefined") {
         if (this.paramConfig.DEBUG_FLAG) console.log("ThemeDetailMysecondappThemeDetailGridGrid ComponentConfig:", ComponentConfig);
...
async KEY_COMMIT(){
  ...
  async ON_CLICK(formGroup, P_INDEX){
     //alert ("Grid Component Trigger: ON_CLICK")
...
async POST_UPDATE(formGroup, P_INDEX){
  console.log("POST_UPDATE:P_INDEX:", P_INDEX)
  //if (P_INDEX == 0) {
    this.applyTheme();
   //}
}
...
applyTheme(){
 let GridData;
  GridData = Object.assign([], this.grid.data);
  let curTheme = {};
  console.log("POST_UPDATE:applyTheme:", GridData.data.length)
  for (let i = 0; i < GridData.data.length; i++) {
    console.log("001 GridData.data[i]",GridData.data[i]);
    let THEME_VARIABLE = "--" + GridData.data[i].THEME_VARIABLE;
    console.log("002 THEME_VARIABLE", THEME_VARIABLE);
    curTheme[THEME_VARIABLE] = GridData.data[i].THEME_VARIABLE_VALUE;
    console.log("003 curTheme:", curTheme);
  }
  // Apply the theme
  Object.entries(curTheme).forEach(([key, value]) => {
    let value1:any = value;
     document.documentElement.style.setProperty(key, value1);
    console.log("004 Theme applied:", key, value);
  });
}
...
async WHEN_VALIDATE_ITEM_THEME_NAME(formGroup) {

 this.FORM_TRIGGER_FAILURE = false ;
 this.formGroup.controls['THEME_NAME'].setErrors({invalid: true});
 // Code goes here
 ...
 this.formGroup.get('THEME_NAME').updateValueAndValidity();
 this.formGroup.updateValueAndValidity();
 }
async WHEN_VALIDATE_ITEM_THEME_VARIABLE(formGroup) {

 this.FORM_TRIGGER_FAILURE = false ;
 this.formGroup.controls['THEME_VARIABLE'].setErrors({invalid: true});
 // Code goes here
 ...
 this.formGroup.get('THEME_VARIABLE').updateValueAndValidity();
 this.formGroup.updateValueAndValidity();
 }
async WHEN_VALIDATE_ITEM_THEME_VARIABLE_VALUE(formGroup) {

 this.FORM_TRIGGER_FAILURE = false ;
 this.formGroup.controls['THEME_VARIABLE_VALUE'].setErrors({invalid: true});
 // Code goes here
 ...
 this.formGroup.get('THEME_VARIABLE_VALUE').updateValueAndValidity();
 this.formGroup.updateValueAndValidity();
 }
 async onBlur_THEME_NAME() {
  await this.WHEN_VALIDATE_ITEM_THEME_NAME(this.formGroup); if ( this.FORM_TRIGGER_FAILURE) return;
 }
 async valueChangeTHEME_VARIABLE(value: any) { 
  await this.WHEN_VALIDATE_ITEM_THEME_VARIABLE(this.formGroup); if ( this.FORM_TRIGGER_FAILURE) return;  
  }
  async onBlur_THEME_VARIABLE_VALUE() {
    await this.WHEN_VALIDATE_ITEM_THEME_VARIABLE_VALUE(this.formGroup); if (this.FORM_TRIGGER_FAILURE) return;
  }
 async valueChangeTHEME_VARIABLE_VALUE(value: any) {
 await this.WHEN_VALIDATE_ITEM_THEME_VARIABLE_VALUE(this.formGroup); if ( this.FORM_TRIGGER_FAILURE) return;
 }
