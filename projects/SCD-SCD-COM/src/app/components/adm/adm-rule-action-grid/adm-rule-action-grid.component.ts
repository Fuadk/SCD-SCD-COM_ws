...
import {   ruleAction , ruleHost, componentConfigDef } from '@modeldir/model';
...
'ACTION_CODE' : new FormControl(dataItem.ACTION_CODE, Validators.required ) ,
...
export class AdmRuleActionGridComponent implements OnInit,OnDestroy {
  showSendTo: boolean=true;
  showMapId: boolean=true ;
  showExtraData: boolean=true ;
  BODY_DATA_textarea = true;
  BODY_DATA_editor = true;
...
private Body =[];
  ...
  constructor(public responsive: BreakpointObserver ,private starlib1: Starlib1,public starServices: starServices, private renderer: Renderer2, private formBuilder: FormBuilder) {
  ...
  private onDocumentClick(e: any): void {
    if (this.formGroup && this.formGroup.valid &&
        !matches(e.target, '#grid tbody *, #grid .k-grid-toolbar .k-button, .k-link')) {
          console.log("Updated Values:onDocumentClick:this.formGroup:",this.formGroup)
        //this.saveCurrent();
    }
  }
...
public openEditorForm(dataItem: any): void {
  const dataItemCopy = { ...dataItem };

  
  this.form = this.formBuilder.group({
    DISABLED: [dataItemCopy.DISABLED],
    MODULE: [{ value: dataItemCopy.MODULE, disabled: !this.isMODULEEnable }, Validators.required],
    RULE_ID: [{ value: dataItemCopy.RULE_ID, disabled: !this.isRULE_IDEnable }, Validators.required],
    ACTION_ID: [{ value: dataItemCopy.ACTION_ID, disabled: !this.isACTION_IDEnable }, Validators.required],
    ACTION_ORDER: [dataItemCopy.ACTION_ORDER, Validators.required],
    ACTION_CODE: [dataItemCopy.ACTION_CODE, Validators.required],
    SEND_TO: [dataItemCopy.SEND_TO],
    MAP_ID: [dataItemCopy.MAP_ID],
    BODY_DATA: [dataItemCopy.BODY_DATA],
    PARAMETER_DATA: [dataItemCopy.PARAMETER_DATA],
    EXTRA_DATA: [dataItemCopy.EXTRA_DATA]
  });

  this.updateFieldVisibility(dataItemCopy.ACTION_CODE);
  this.fillHelpValue();
  this.opened = true;
}

private updateFieldVisibility(actionCode: string): void {
  this.showSendTo = true;
  this.showMapId = true;
  this.showExtraData = false; 
  this.BODY_DATA_textarea = true;
  this.BODY_DATA_editor = false;
  console.log("actionCode:", actionCode)
  switch (actionCode) {
      case 'EMAIL':
          this.showSendTo = false;
          this.showMapId = false;
          this.BODY_DATA_editor = true;
          this.BODY_DATA_textarea = false;
          break;
      case 'SMS':
          this.showSendTo = false;
          this.showMapId = false;
...
public async onDelete(dataItem: ruleAction): Promise<void> { 
  await this.starServices.sleep(200);
  this.removeButton(this.grid);
  return;
...
public onSave(): void {
  if (this.form.valid) {
    let updatedValues = this.form.value;
    
    console.log("arr1.length:updatedValues.BODY_DATA:", updatedValues.BODY_DATA);
    let arr1= updatedValues.BODY_DATA.split('<span style="color: red;">');
    console.log("arr1.length:", arr1.length);
    if (arr1.length > 1){
      updatedValues.BODY_DATA = updatedValues.BODY_DATA.split('<span style="color: red;">').join("");
      updatedValues.BODY_DATA = updatedValues.BODY_DATA.split('</span>').join("");
    }
    var form_DSP_EDITOR_txt = this.html2Text(updatedValues.BODY_DATA);
    let errMsg = this.validateInput(form_DSP_EDITOR_txt);
    if (errMsg != ""){
      return;
    }

    errMsg = this.validateInput(updatedValues.PARAMETER_DATA);
    if (errMsg != ""){
      return;
    }
    this.formGroup.patchValue( updatedValues);
    ...
    this.opened = false;
     ...
     this.starServices.getInvalidControls(this);
...
public onCancel(): void {
  ...
  public addButton (){
    ...
    console.log("addButton:", this.grid.data)
    let  formInitialValues:any =   new ruleAction(); 
    formInitialValues.MODULE = this.masterKeyArr[0];
    formInitialValues.RULE_ID = this.masterKeyArr[1];
    formInitialValues.ACTION_ID = 1;
    formInitialValues.ACTION_ORDER = 1;
    let GridData:any = this.grid.data;
    
    let maxOrder= 1;
    for (let i=0; i < GridData.data.length ; i++){
      if (GridData.data[i].ACTION_ID >= formInitialValues.ACTION_ID)
        formInitialValues.ACTION_ID = parseInt( GridData.data[i].ACTION_ID) + 1;
      if (GridData.data[i].ACTION_ORDER >= formInitialValues.ACTION_ORDER)
        formInitialValues.ACTION_ORDER = parseInt(GridData.data[i].ACTION_ORDER ) + 1;

    }

    this.openEditorForm(formInitialValues);
    
  }
  public fillHelpValue(){
    this.helpValue = "";
    let lkpArrFIELD = this.paramConfig.lkpArrFIELD;
   
    //this.helpValue = "<h5>Fields to use :</h5>"
    Object.keys(lkpArrFIELD).map(key => {
      var val = lkpArrFIELD[key];
      console.log("val:", val);
      if (val.CODE != "ORDER_FIELDS")
        if (val.CODE != "")
          //this.helpValue = this.helpValue  + "::" + val.CODE + "::<br/>";
          this.helpValue = this.helpValue  + "::" + val.CODE + "::\n";
 

    });
    console.log("this.helpValue:", this.helpValue);
  }
 public cellClickHandler({ column, isEdited, dataItem, rowIndex }): void {
  ...
  if ( (fieldName == "PARAMETER_DATA")  || (fieldName == "BODY_DATA") ) {
    ...
    if (fieldName == "BODY_DATA")
      this.DSP_EDITORFormConfig.title = "Body Data";
    if (fieldName == "PARAMETER_DATA")
      this.DSP_EDITORFormConfig.title = "Parameter Data";
    this.fillHelpValue();
   
     this.editorFormOpened =  true;
     if (this.paramConfig.DEBUG_FLAG) console.log("this.DSP_EDITORFormConfig:", this.DSP_EDITORFormConfig);
  }
  if (this.paramConfig.DEBUG_FLAG) console.log("Updated Values:cellClickHandler:isEdited",isEdited, this.formGroup)
  ...
  public buildStr(gridVal){
    ...
    if (this.paramConfig.DEBUG_FLAG) console.log("gridVal.FIELD:" + gridVal.ACTION_CODE)
        ...
        if(gridVal.BODY_DATA != "" && gridVal.BODY_DATA != null)
            str  = str  + "....... BODY: .......  " + this.html2Text(gridVal.BODY_DATA);
  ...
  public errMsg ="";
  public errorFormOpened = false;
  
  public validateInput(bodyData){
      this.errMsg = "";
      var array = bodyData.split("::");
			//console.log("Vlidate:array:", array, " array.length:", array.length);
      let  i = 1;
			while ( i < array.length)
			{
				var paramData = "::" + array[i] + "::"
        
        var rec = this.paramConfig.lkpArrFIELD.find(x => x.CODE === array[i]);
        //console.log ("Vlidate:rec:", rec)
        if (typeof rec == "undefined"){
          this.errMsg = "Not a valid field : " + array[i];
          this.errorFormOpened = true;
          break;
        }
				
				//console.log("Vlidate:paramData:"+ paramData);
				array[i] = paramData;
        i = i + 2;
			}
      return this.errMsg;
  ...
  public saveFormCompletedHandler( form_DSP_EDITOR) {
    ...
    var form_DSP_EDITOR_txt = this.html2Text(form_DSP_EDITOR);
    let errMsg = this.validateInput(form_DSP_EDITOR_txt);
    if (errMsg != ""){
