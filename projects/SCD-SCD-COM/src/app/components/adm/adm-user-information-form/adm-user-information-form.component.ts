...
'THEME_NAME' : new FormControl(dataItem.THEME_NAME ),
...
private Body =[];
  ...
  public async ngOnInit() {
    ...
    this.form.get('DIVS').valueChanges.subscribe(val => {
      if (this.paramConfig.DEBUG_FLAG) console.log("valu changed :DIVS :" + val + " :this.form.value:",this.form.value)
  ...
  public lookupArrDef =[   
    ...
    {"statment":"select THEME_NAME CODE, THEME_NAME CODETEXT_LANG from THEME_DEF ",
    "lkpArrName":"lkpArrTHEME_NAME"
    },
    {"statment":"SELECT CODE  , CODETEXT_LANG FROM SOM_TABS_CODES WHERE CODENAME ='COMPANY' and LANGUAGE_NAME = '" + this.userLang + "' order by CODETEXT_LANG ",
    //{"statment":"select COMPANY_ID CODE, COMPANY_NAME CODETEXT_LANG from ATN_COMPANY order by COMPANY_NAME ",
    "lkpArrName":"lkpArrCOMPANY_ID"
    }                       
  ];
  public lkpArrCOMPANY_ID = [];
  public lkpArrGROUPNAME = [];
  public lkpArrDEPT = [];
  public lkpArrTHEME_NAME = [];
