...
const createFormGroup = dataItem => new FormGroup({
  ...
  'MENU_TEXT': new FormControl(dataItem.MENU_TEXT),
  ...
  'HIDDEN': new FormControl(dataItem.HIDDEN),
...
private Body = [];
  ...
  public orgisMaster: boolean = false;
  ...
  @Output() removeCompletedOutput: EventEmitter<any> = new EventEmitter();
  ...
  public setInitialValues() {
    this.form.patchValue({ 'HIDDEN': '0' });
    console.log("setInitialValues:", this.form.value)

  }
  @Input() public set executeQueryInput(form: any) {
    if (this.paramConfig.DEBUG_FLAG) console.log("executeQuery_form object.form:", form, this.form);
  ...
  async callBackPost_Remove( NewVal:any) {
      ...
      this.isMaster = this.orgisMaster;
      this.removeCompletedOutput.emit(NewVal);
   }

  public onRemove(form): void {
    this.orgisMaster = this.isMaster;
    this.isMaster = false;
