...
private Body = [];
  ...
  constructor(private starNotify: StarNotifyService, public responsive: BreakpointObserver ,public starServices: starServices, private renderer: Renderer2) {
  ...
  public ngAfterViewInit() {
    ...
    if (this.paramConfig.defaultModule == "FLOW"){
      this.lkpArrMODULE = this.lkpArrMODULE.find(x => x.CODE === "FLOW");
      this.valueChangeMODULE("FLOW");
    }
  ...
  public executeQuery(MODULE: any): void {
    ...
    this.starServices.fetch(this, page).subscribe(result => {
      ...
      if (result != null) {
        ...
        this.treeDataArr = this.buildTree(result.data[0].data);
        if (this.paramConfig.DEBUG_FLAG) console.log("trace:this.treeDataArr:", this.treeDataArr)
        if (this.paramConfig.DEBUG_FLAG) console.log("this.selectedKeys[0]:", this.selectedKeys)

        this.get_ids(this.treeDataArr);
        ...
        var treeID = 0;
        if (typeof this.treeDataArr !== "undefined") {
          if (this.currentOrder != null) {
            var i = 0;
            while (i < this.treeDataArr.length) {
              if (this.paramConfig.DEBUG_FLAG) console.log("currentOrder:", this.treeDataArr[i].order, this.currentOrder)
              if (this.treeDataArr[i].order == this.currentOrder) {
                treeID = i;
                break;
              }
              i++;
        ...
        if (this.selectedKeys.length == 0){
          ...
          this.ADM_RULE_DEFFormConfig = new componentConfigDef();
          this.ADM_RULE_DEFFormConfig.formattedWhere = Page;
          index = -1;
          itemType = "APP";
  ...
  public actonSelection(i) {
    ...
    // }
    // else if (this.starServices.LKP_LIKE.includes(this.serverData[this.selectedKeys[0]].ITEM_TYPE)) {
    ...
    // this.ADM_RULE_DEFFormConfig = new componentConfigDef();
    // this.ADM_RULE_DEFFormConfig.masterKeyArr = [this.serverData[i].MODULE, this.serverData[i].ITEM_MODULE, this.cdrType2, this.serverData[i].ITEM_PARAM3];
    // this.ADM_RULE_DEFFormConfig.masterKeyNameArr = ["MODULE", "CDR_TYPE_1", "CDR_TYPE_2", "JOIN_DEF"];
    // this.ADM_RULE_DEFFormConfig.masterParams = masterParams;
  ...
  this.lookupArrDef =[
    {"statment":"select CHOICE CODE, MENU_TEXT CODETEXT_LANG from MENUS m where MENU  in (" + this.paramConfig.licensedModules + ") and LANGUAGE_NAME = '" + this.userLang + "' order by CODETEXT_LANG ",
  ...
  public saveCompletedHandler(form_ADM_RULE_DEF_TREE) {
    ...
    this.ADM_RULE_DEFFormConfig = new componentConfigDef();
    this.ADM_RULE_DEFFormConfig.masterSaved = form_ADM_RULE_DEF_TREE;
  ...
  WHEN_NEW_FORM_INSTANCE() {
    ...
    // this.ADM_RULE_DEFFormConfig = new componentConfigDef();
    // this.ADM_RULE_DEFFormConfig.formattedWhere = Page;
    // this.ADM_RULE_DEFFormConfig.masterReadCompleted =true;
  }
  public showHideGrid(){
  ...
  public showRuleDef(){
    this.showRules = true;
    this.ADM_RULE_DEFFormConfig = new componentConfigDef();
      this.ADM_RULE_DEFFormConfig.clearScreen = true;
