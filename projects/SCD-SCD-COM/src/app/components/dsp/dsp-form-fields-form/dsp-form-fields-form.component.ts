...
private Body:any = [];
  ...
  public DSP_Form_FIELDS_RADIOGridConfig: componentConfigDef;
  ...
  public async ngOnInit() {
    ...
    if (this.paramConfig.DEBUG_FLAG) console.log ("this.form:invalid:", this.form.invalid)

    this.DSP_Form_FIELDS_RADIOGridConfig = new componentConfigDef();
    this.DSP_Form_FIELDS_RADIOGridConfig.isChild = true;
    this.DSP_Form_FIELDS_RADIOGridConfig.gridHeight = "450";
  ...
  @Input() public set executeQueryInput(form: any) {
    if (this.paramConfig.DEBUG_FLAG) console.log ("this.form:form:", form)
    if ((typeof form != "undefined") && form.type == "field" && (typeof form.data.field.FORM_NAME != "undefined") && (form.data.field.FORM_NAME != "")) {
      ...
      // Fuad: 2 null below added as where is like % cause no data (sometimes) seems related to miss order of fields
      formData.field.FIELD_ROW =null
      formData.field.FIELD_ORDER =null
//      this.form.setValue(formData.field);
//      this.form.updateValueAndValidity(); 
      if (this.paramConfig.DEBUG_FLAG) console.log ("this.form:formData.field):", this.form.invalid, formData.field)
...
this.form.get('FIELD_LOOKUP').valueChanges.subscribe(val => {
  ...
  @Input() public set setComponentConfig_Input(ComponentConfig: componentConfigDef) {
    if (typeof ComponentConfig !== "undefined") {
      ...
      if (ComponentConfig.masterSaved != null && ComponentConfig.masterSaved == true) {
        ...
        setTimeout(() => {
          if (this.paramConfig.DEBUG_FLAG) console.log ("this.form:this.form:", this.form)
          this.saveChanges(this.form);
  ...
  inputClicked(id, type) {
    if (this.paramConfig.DEBUG_FLAG) console.log("inputClicked:", id, type)
    ...
    if (this.paramConfig.DEBUG_FLAG) console.log("FIELD_TYPE:", FIELD_TYPE)
    ...
    if (FIELD_TYPE == "TEXT_AREA" || FIELD_TYPE == "HTML" && (type == "editor" || type == "help")) {
      ...
      this.editorFormOpened = true
      ...
      else if (FIELD_TYPE == "RADIO" && id == "FIELD_DEFAULT" ){
        console.log("dataItem:", dataItem)
        var masterKeyArr = [dataItem.FORM_NAME, dataItem.PAGE_NO, dataItem.AREA_NO,  dataItem.FIELD_ID];
        var masterKeyNameArr = ["FORM_NAME", "PAGE_NO", "AREA_NO", "FIELD_ID"];
        ...
        this.DSP_Form_FIELDS_RADIOGridConfig = new componentConfigDef()
        this.DSP_Form_FIELDS_RADIOGridConfig.masterParams = masterParams
        this.DSP_Form_FIELDS_RADIOGridConfig.masterKeyArr = masterKeyArr
        this.DSP_Form_FIELDS_RADIOGridConfig.masterKeyNameArr = masterKeyNameArr;
        this.DSP_Form_FIELDS_RADIOGridConfig.masterReadCompleted=true;
        this.DSP_Form_FIELDS_RADIOGridConfig.title = "Value"
        this.showRadioGrid = true
      }
  }
  ...
  saveFormCompletedHandlerRadio(value){
  ...
  public showRadioGrid = false;
  public showRadioGridClose(){
    this.showRadioGrid = false;
  }
  
  public showSomeCode = false;
  public showSomCodes(){
    this.showSomeCode = true;
  }
