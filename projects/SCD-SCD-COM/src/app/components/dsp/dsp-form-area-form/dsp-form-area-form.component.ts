...
const createFormGroup = (dataItem:any) => new FormGroup({
  ...
  'AREA_TYPE': new FormControl(dataItem.AREA_TYPE, Validators.required),
  ...
  @Input() public set setComponentConfig_Input(ComponentConfig: componentConfigDef) {
    if (typeof ComponentConfig !== "undefined") {
      ...
      if (ComponentConfig.masterParams != null) {
        this.formInitialValues.AREA_NO = ComponentConfig.masterParams.maxAreaNo+1;
      }            
  ...
  async onValueChange_AREA_TYPE(value) { 
   ...
   //if ( this.FORM_TRIGGER_FAILURE) return;  
    ...
    async WHEN_VALIDATE_ITEM_AREA_TYPE(value) {
      ...
      // Code goes here 
      if (value == "FORM")
        this.form.patchValue({ 'AREA_DATA_NAME': 'formArr' });
      if (value == "GRID")
        this.form.patchValue({ 'AREA_DATA_NAME': 'gridArr' });
