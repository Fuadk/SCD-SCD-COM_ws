...
private Body:any = [];
  ...
  constructor(public responsive: BreakpointObserver ,public starServices: starServices, private starlib1: Starlib1,) {
  ...
  async ngOnInit() {
    ...
    this.DSP_ORDERSGridConfig.gridHeight = 350;
  ...
  public executeQuery(form: any): void {
    ...
    // form["ASSIGNEE"] = this.starServices.sessionParams.USER_INFO.USERNAME
    let orderType =  this.form.value["ORDER_TYPE"];
    console.log("orderType:", orderType)
    if ( (orderType == "") || (typeof orderType == "undefined")) 
      orderType ="%";
    let whereClause = "( (ASSIGNEE ='" + this.starServices.USERNAME + "' )"
            + " OR (ORDER_NO in "
            + "      (SELECT DISTINCT A.ORDER_NO "
            + "         from DSP_WORK_ORDERS A, DSP_ORDERS B, ADM_USER_INFORMATION C , ADM_USER_INFORMATION D "
            + "       WHERE "
            + "          (  ( B.ASSIGNEE = '' OR C.USERNAME = B.ASSIGNEE   ) "
            + "             AND A.ORDER_NO = B.ORDER_NO "
            + "             AND C.DEPT = D.DEPT "
            + "             AND C.USERNAME != D.USERNAME "
            + "             AND  D.MANAGER = 1 and A.ASSIGNEE_TYPE = 'ROLE' and A.ASSIGNEE = 'MGR' AND D.USERNAME ='" + this.starServices.USERNAME + "' "
            + "           ) "            
            + "          OR  ( A.ASSIGNEE_TYPE ='TEAM' AND A.ASSIGNEE ='" + this.starServices.USER_INFO.DEPT + "' )"
            + "          OR  ( A.ASSIGNEE_TYPE ='PERSON' AND A.ASSIGNEE ='" + this.starServices.USERNAME + "' )"
            + "      ) "
            + "     ) )";
...
this.form.get('ASSIGNEE_TYPE').valueChanges.subscribe(val => {
  ...
  //assignee = this.form.value["ASSIGNEE"];
   assignee = this.starServices.USERNAME;
   let dept = this.starServices.USER_INFO.DEPT;
    ...
    let params = {
      ...
      ASSIGNEE:assignee,
      DEPT : dept,
      USERNAME: this.starServices.USERNAME
  ...
  public showMultiStepForm() {
    ...
    var newVal:any = { "_QUERY": "GET_DSP_TEMPLATE", 
                            "TEMPLATE_NAME": this.addForm.value.TEMPLATE_NAME };
    this.Body.push(newVal);
