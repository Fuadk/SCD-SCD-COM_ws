...
//Next part for filtering
  ...
  public async getsendWOs(ORDER_NO){
    ...
    Orders = data[1].data;
    if (this.paramConfig.DEBUG_FLAG) console.log("testx post execSQLBody:", workOrders);
      if (workOrders.length != 0) {
        ...
        if (this.paramConfig.DEBUG_FLAG) console.log("testx post body:", body);
      let data = await this.starServices.execSQLBody(this, body, "");
        ...
        if (newGridData.data[i].PARENT_WO_ORDER_NO == PARENT_WO_ORDER_NO ) {
          if (newGridData.data[i].WO_STATUS  == paramConfig.CREATED ) {
            ...
            var NewVal = {
              ...
              "ORDER_NO" : newGridData.data[i].ORDER_NO,
...
@Input() public set setComponentConfig_Input(ComponentConfig: componentConfigDef) {
  ...
  public colorCode(code: string) {
    ...
    if (typeof rec !== "undefined") 
      result = rec.PARTCODE;
    if (this.paramConfig.DEBUG_FLAG) console.log("colorCode:result:", result)
    return (result);
  }
}
