  ...
  */
  ...
  private createFormGroup(fields: any = {}) {
    ...
    for (var key in fields) {
      if (this.paramConfig.DEBUG_FLAG) console.log("test4:key:",key);
      var val = fields[key];
      if (this.paramConfig.DEBUG_FLAG) console.log("test4:fields[key]:",fields[key], Validators.required);
    ...
    if (this.paramConfig.DEBUG_FLAG) console.log("test4:rec1:", rec1)
    rec1.markAllAsTouched()
    setTimeout(() => {
      this.formValidationChangedOutput.emit(rec1.valid)
    }, 100)
  ...
  public cellClickHandler({ column, isEdited, dataItem, rowIndex }): void {
    if (isEdited  || this.areaProtected == 1) {
      return;
    }
    console.log("this.ComponentConfig:updateable", this.componentConfig.updateable);
    if (!this.componentConfig.updateable)
      return;
  ...
  public getlkpArrField(fieldName) {
    ...
    var templateFieldsRec = this.getFieldsRec(fieldName, this.templateFields);
    if (templateFieldsRec != null) {
      if ( (typeof this.lkparr != "undefined") && (typeof this.lkparr[templateFieldsRec.FIELD_LOOKUP] != "undefined")) {
        let lkpArr = this.lkparr[templateFieldsRec.FIELD_LOOKUP];
        return lkpArr;
      }
      else
  ...
  public showGridData(jsonData, templateFields) {
    ...
    for (var i = 0; i < templateFields.length; i++) {
      ...
      var fieldProtected = false;
      if (typeof templateFields[i].FIELD_PROTECTED !== "undefined") {
        fieldProtected = templateFields[i].FIELD_PROTECTED;
      }
      var fieldWidth = 100;
      if (typeof templateFields[i].FIELD_WIDTH !== "undefined") {
        fieldWidth = templateFields[i].FIELD_WIDTH;
      }
...
*/
    ...
    for (let i = 0; i < this.gridData.length; i++){
      this.formGroup = this.createFormGroup(this.gridData[i]);
    }
  ...
  public printScreen() {
    ...
    if (typeof ComponentConfig !== "undefined") {
      ...
      console.log("this.ComponentConfig:", this.componentConfig.updateable);
      ...
      if (ComponentConfig.masterParams != null) {
        ...
        if (ComponentConfig.masterParams.areaProtected !== null)
         this.areaProtected = ComponentConfig.masterParams.areaProtected;
        
        if (typeof ComponentConfig.masterParams.lkparr !== "undefined")
          this.lkparr = ComponentConfig.masterParams.lkparr;
        if (this.areaProtected)
          this.componentConfig.showToolBar = (this.areaProtected == 0) ?  true:false;
