...
public prepareReportSQLParams(rec){
  var sql = "SELECT " + rec.SELECT_CLAUSE + " FROM " + rec.FROM_CLAUSE ;
  if ((rec.WHERE_CLAUSE != "") && ( rec.WHERE_CLAUSE !== null ) )
    sql = sql + " WHERE " + rec.WHERE_CLAUSE;
...
public series: any[] = [
  ...
  public categories = ["ADU", "INST"];
*/
public BAR_LIKE = ["bar",  "column", "stack_bar", "area", "verticalArea"];
public PIE_LIKE = ["pie", "line","donut", "funnel", "waterfall"];
...
public processBarLikeChartData() {
  ...
  for (var dataNamekey in dataNames) {
      ...
      if (this.paramConfig.DEBUG_FLAG) console.log("dataNamekey:", dataNamekey, " dataNameval:", dataNameval);
      var dataName = {};
