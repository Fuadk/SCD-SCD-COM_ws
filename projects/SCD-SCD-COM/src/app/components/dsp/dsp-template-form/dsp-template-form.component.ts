...
const createFormGroup = (dataItem:any) => new FormGroup({
  ...
  'FORM_TYPE' : new FormControl(dataItem.FORM_TYPE , Validators.required) ,
  'HIDE_PRV_STEPS' : new FormControl(dataItem.HIDE_PRV_STEPS ) ,
...
private Body:any =[];
  ...
  public async ngOnInit() {
    ...
    //this.formInitialValues.ASSIGNEE = this.starServices.sessionParams.USER_INFO.TEAM;
...
this.form.get('ASSIGNEE_TYPE').valueChanges.subscribe(val => {
    ...
    this.form.get('FORM_TYPE').valueChanges.subscribe(value => {
      if (value == "FORM")
        this.lkpArrSCREEN_FORM_NAME = this.lkpArrFORM_NAME;
      if (value == "SCREEN")
        this.lkpArrSCREEN_FORM_NAME = this.lkpArrSCREEN_NAME;
      console.log("valueChanges:", this.lkpArrSCREEN_FORM_NAME)
    });
  }
  ...
  public onRemove( form:any): void {
    if (form.value.TEMPLATE_OWNER != "" && this.starServices.sessionParams.USER_INFO.USERNAME.toUpperCase() != form.value.TEMPLATE_OWNER.toUpperCase()) {
      this.starServices.showOkMsg(this, "You are not the owner of this template, but :" + form.value.TEMPLATE_OWNER, "Error");
      return;
    }
  ...
  public saveChanges( form: any): void {
    ...
    //console.log("this.starServices.sessionParams.USER_INFO.USERNAME:", this.starServices.sessionParams.USER_INFO.USERNAME,"form.value..TEMPLATE_OWNER:", form.value.TEMPLATE_OWNER, " x");
      if (form.value.TEMPLATE_OWNER == "")
        form.value.TEMPLATE_OWNER = this.starServices.sessionParams.USER_INFO.USERNAME;

      // if (this.starServices.sessionParams.USER_INFO.USERNAME.toUpperCase() != form.value.TEMPLATE_OWNER.toUpperCase()) {
      //   this.starServices.showOkMsg(this, "You are not the owner of this template, but :" + form.value.TEMPLATE_OWNER, "Error");
      //   return;
      // }
...
this.multiStepFormOpened = false; 
  ...
  this.lookupArrDef =[	{"statment":"SELECT CODE,  CODETEXT_LANG FROM SOM_TABS_CODES WHERE CODENAME ='ORDER_TYPE' and LANGUAGE_NAME = '" + this.userLang + "' order by CODETEXT_LANG  ",
 ...
 {"statment":"SELECT CODE, CODETEXT_LANG FROM SOM_TABS_CODES WHERE CODENAME ='ASSIGNEE_TYPE' and LANGUAGE_NAME = '" + this.userLang + "' order by CODETEXT_LANG  ",
      ...
      {"statment":"SELECT  CHOICE CODE, MENU_TEXT CODETEXT_LANG FROM MENUS where LANGUAGE_NAME = '" + this.userLang + "' " 
        + " and MENU_TYPE ='R' and CHOICE_TYPE ='ST' "
        + " order by CODETEXT_LANG "
        ,
        "lkpArrName":"lkpArrSCREEN_NAME"}
  ...
  public lkpArrSCREEN_NAME = [];
  public lkpArrSCREEN_FORM_NAME =[];
  public lkpArrFORM_TYPE = [
    {
      CODE:"FORM",
      CODETEXT_LANG:"Form"
    },
  ...
  public onValueChange_FORM_TYPE(value)
  {
    console.log("onValueChange_FORM_TYPE:",value )
  }
  getLkpArrFORM_NAME(){
    let value = this.form.value['FORM_TYPE']
    console.log("onValueChange_FORM_TYPE:",value )
    if (value == "FORM")
      return this.lkpArrFORM_NAME;
    if (value == "SCREEN")
      return this.lkpArrSCREEN_NAME;
  }
  saveGridDataToTemplate(data) {
    ...
    this.addToBody({
      ...
      "TEMPLATE_OWNER": this.form.value["TEMPLATE_OWNER"],
      "FORM_TYPE": this.form.value["FORM_TYPE"],
