...
private Body = []
  ...
  constructor(public responsive: BreakpointObserver ,public starServices: starServices, public dialogService: DialogService) {
  ...
  async ngOnInit() {
   ...
   //this.starServices.fetchLookups(this, this.lookupArrDef);
    ...
    this.saveCompletedOutput.subscribe(data => {
      if (this.paramConfig.DEBUG_FLAG) console.log("saveCompletedOutput:data.Body:", data.Body)
      if (data.Body) {
        if ( ( data.Body.length > 0 ) && (data.Body[0]["FIELD_ROW"])) {
          ...
          if (this.paramConfig.DEBUG_FLAG) console.log("ELHAMWAA")
  ...
  public saveCompletedHandler(form_DSP_FORM_DEF) {
    if (this.paramConfig.DEBUG_FLAG) console.log("this.form:form_DSP_FORM_DEF:", form_DSP_FORM_DEF)
    if (form_DSP_FORM_DEF.type == "page_new") {
      if ( (form_DSP_FORM_DEF.data.PAGE_NO == "") ||
           (form_DSP_FORM_DEF.data.PAGE_ORDER == "") ||
           (form_DSP_FORM_DEF.data.PAGE_TYPE == "") 
    )
      return;
    }
    if (form_DSP_FORM_DEF.type == "area_new") {
      if ( (form_DSP_FORM_DEF.data.AREA_NO == "") ||
           (form_DSP_FORM_DEF.data.AREA_TYPE == "") 
    )
      return;
    }
    ...
    if (form_DSP_FORM_DEF.PAGE_TITLE || form_DSP_FORM_DEF.type == "page") {
      ...
      if ((form_DSP_FORM_DEF.PAGE_NO ?? form_DSP_FORM_DEF.data["PAGE_NO"]) == this.currentPages[this.currentPage].pageNo) {
        ...
        if (fieldArea) {
          ...
          //     content: "At least one field should be checked as required. Please select a field and check REQUIRED check Box.",
          //     actions: [
  ...
  selectElement(type, data = null, checkAutoSave = true) {
    ...
    if (this.paramConfig.DEBUG_FLAG) console.log("this.form:",type, data, this.currentForm)
    ...
    setTimeout(() => {
      ...
      if (this.paramConfig.DEBUG_FLAG) console.log("this.form:type:",type, "field:1:", data.field, { ...data.field })
      if (type == "field") {
        let field = {
          isNew: data.isNew,
          field: { ...data.field }
        }
        if (this.paramConfig.DEBUG_FLAG) console.log("this.form:type:",type, "field:2:", data.field)
        if (this.paramConfig.DEBUG_FLAG) console.log ("this.form:selectElement:field.field.FIELD_ROW_NEW:", field.field.FIELD_ROW_NEW, field)
        ...
        if (this.paramConfig.DEBUG_FLAG) console.log ("this.form:selectElement:field.field.FIELD_DEFAULT_NEW:", field.field.FIELD_DEFAULT_NEW, field)
        ...
        if (this.paramConfig.DEBUG_FLAG) console.log ("this.form:data.isNew):", data.isNew)
        if (data.isNew) {
  ...
  onDrop(e) {
    ...
    this.currentFields.forEach((row: [], rowIndex) => {
      row.forEach((field: any, order) => {
        if (this.paramConfig.DEBUG_FLAG) console.log("herex2:FIELD_ORDER_NEW:", field.field.FIELD_ID, field.field["FIELD_ROW_NEW"], field.field["FIELD_ORDER_NEW"] )
        if (typeof field.field["FIELD_ORDER_NEW"] !== "undefined"){
          if (this.paramConfig.DEBUG_FLAG) console.log("herex2:FIELD_ORDER_NEW")
          field.field["FIELD_ORDER_NEW"] = order + 1
          field.field["FIELD_ROW_NEW"] = rowIndex + 1
        }
      })
    })
    if (this.paramConfig.DEBUG_FLAG) console.log("herex2:this.currentFields:", this.currentFields)
    let saveData = [].concat.apply([], this.currentFields).filter((f: any) => !f.isNew).map((f: any) => {
      f.field._QUERY = "UPDATE_DSP_FORM_FIELDS"
      if (this.paramConfig.DEBUG_FLAG) console.log("herex2:FIELD_ORDER_NEW:", f.field.FIELD_ORDER_NEW, "FIELD_ROW_NEW:", f.field.FIELD_ROW_NEW)
  ...
  async callBackPost_Save( NewVal) {
    ...
    if (this.currentAreas.length > 0)
      this.selectArea(this.currentAreas[this.currentArea], this.currentArea, false)

  }
  public saveCurrent(): void {
    if (this.paramConfig.DEBUG_FLAG) console.log("herex1")
  ...
  addArea() {
    let maxAreaNo = 0;
      if (typeof this.currentAreas[0] == "undefined")
        maxAreaNo = 0;
      else{
        maxAreaNo = this.currentAreas[0].AREA_NO;
        if (this.currentPage + 1 != this.currentAreas[0].PAGE_NO)
          maxAreaNo = 0;
      }
    var masterParams = {
      "maxAreaNo": maxAreaNo
    ...
    this.DSP_AREAConfig = new componentConfigDef();
    this.DSP_AREAConfig.masterParams = masterParams;
  ...
  addPage(formName = null) {
    ...
    //console.log("this.currentPages:", this.currentPages[0].data)
    let maxPageNo = 0;
    let maxPageORder = 0;
    for (let i =0; i< this.currentPages.length; i++){
      if (this.currentPages[i].data.PAGE_NO > maxPageNo)
        maxPageNo = this.currentPages[i].data.PAGE_NO;
      if (this.currentPages[i].data.PAGE_ORDER > maxPageORder)
        maxPageORder = this.currentPages[i].data.PAGE_ORDER;
    }
    var masterParams = {
      "maxPageNo": maxPageNo,
      "maxPageORder": maxPageORder
    };
    this.DSP_PAGEConfig = new componentConfigDef();
    this.DSP_PAGEConfig.masterParams = masterParams;
    this.DSP_PAGEConfig.gridHeight = this.fieldGridHeight;
    this.DSP_PAGEConfig.formMode = "page_new";
  ...
  async callBackPost_Remove( NewVal:any) {
    if (NewVal._QUERY == "DELETE_DSP_FORM_PAGE"){
      if (this.paramConfig.DEBUG_FLAG) console.log("deletePage:this.currentPage: ", NewVal, this.currentPage, this.currentPages)
      let i = 0;
      while (i < this.currentPages.length){
        if (this.currentPage+1 == this.currentPages[i].pageNo){
          this.currentPages.splice(i, 1);
          break;
        }
        i++;
      }
  ...
  deleteArea() {
    ...
    if (this.paramConfig.DEBUG_FLAG) console.log("deleteArea")
  ...
  deleteField(field, column, index, e) {
    ...
    if (!field.isNew) {
      ...
      if (this.paramConfig.DEBUG_FLAG) console.log("herex3")
      this.insertCMD = "INSERT_DSP_FORM_FIELDS";
      this.updateCMD = "UPDATE_DSP_FORM_FIELDS";
      this.deleteCMD = "DELETE_DSP_FORM_FIELDS";
      this.getCMD = "GET_DSP_FORM_FIELDS_QUERY";
  ...
  saveNewField() {
    ...
    this.newFieldId = this.newFieldId.split(" ").join("_");;
    this.newField.FIELD_ID = this.newFieldId.trim();
  ...
  doCopyForm(form, pages, areas, fields) {
    ...
    if (this.paramConfig.DEBUG_FLAG) console.log("doCopyForm")
