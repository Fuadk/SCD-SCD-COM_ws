...
export class DspTemplateDetailGridComponent implements OnInit,OnDestroy {
 ...
 public showDiagram = true;
  ...
  public title = "Work Flow Detail";
  public PDFfileName = this.title + ".PDF";
  public ExcelfileName = this.title + ".xlsx";
  public compSelector = 'app-dsp-template-detail-grid';
  public children = ["any"];
...
private Body:any =[];
  ...
  constructor(public responsive: BreakpointObserver ,public starServices: starServices, 
              ...
              private router: Router, private starNotify: StarNotifyService) { 
...
//Next part for filtering
  ...
  */
  @Input() public set detail_Input(grid: any) {
    if ( (typeof grid != "undefined") && (grid.TEMPLATE_NAME != "") && (typeof grid.TEMPLATE_NAME != "undefined") )
    {
      ...
      this.templateName = grid.TEMPLATE_NAME;
  ...
  public setInitialValues() {
    this.starServices.setIdOrder(this,"SEQUENCE_NAME", "TEMPLATE_ORDER")
  }
 public cellClickHandler( event:any ): void {
  ...
  public async sendToDiagramWrapper(data) {
    console.log("sendToDiagramWrapper:data:",data)
    let rulesDef =[];
    let templateName = "";
    if (typeof data.data[0] !== "undefined"){
      templateName = data.data[0].TEMPLATE_NAME
      this.fillLkps(data.data);
        
      let whereClauseRules = " TEMPLATE_NAME = '" + data.data[0].TEMPLATE_NAME + "'";
      let body = [
        {
      ...
      let ruledata = await this.starServices.execSQLBody(this, body, "");
      rulesDef = ruledata[0].data;
    }
  ...
  public async callBackFunction(data) {
    // if (data.total === 0)
    //    return;

     this.sendToDiagramWrapper(data)
  ...
  public saveChanges(grid: GridComponent): void {
    ...
    this.sendToDiagramWrapper(this.grid.data);
  }
    ...
    if (typeof GridData.data !== "undefined"){
      GridData.data.sort(function (a, b) {
        ...
        if (a.TEMPLATE_ORDER < b.TEMPLATE_ORDER)
          cmp = -1;
        console.log("a:b:", a.TEMPLATE_ORDER, b.TEMPLATE_ORDER, cmp);
        return cmp;
      });
    }

    this.fillLkps(GridData.data);

    let whereClauseRules = " TEMPLATE_NAME = '" + this.templateName + "'";
    let body = [
      {
        "_QUERY": "GET_ADM_RULE_DEF_QUERY",
        "_WHERE": whereClauseRules
  ...
  public showRules(data){
    console.log("showRules:data:", data)
    var whereClause = "TEMPLATE_NAME  = '" + data.TEMPLATE_NAME + "' ";
    ...
    let masterParams = {
      ...
      formattedWhere_ruleLog: "",
    }
    this.ADM_RULESConfig.masterParams = masterParams;
    this.ADM_RULESConfig.showHideGridFlag = false
  ...
  async showMultiStepForm(dataItem){
    ...
    let Body=[];
    let newVal = { "_QUERY": "GET_DSP_TEMPLATE", "TEMPLATE_NAME": this.templateName };
    Body.push(newVal);
    let data = await this.starServices.execSQLBody(this, Body,"");
    ...
    if ( (formName!= "") && ( (formPagesNo != 0) && (formPagesNo != "") && (formPagesNo != null))  ){
        ...
        if (this.paramConfig.DEBUG_FLAG) console.log("test:masterParams:", masterParams,"templateInfo:",templateInfo);
  ...
  console.log("Checking:CODE:lkpArrGetWO_TYPE:", CODE,rec, this.lkpArrGetWO_TYPE )
  return rec;
  }
  ...
  public lkpArrGetDETAIL(CODE: any): any {
    
    // Change x.CODE below if not from SOM_TABS_CODE
    CODE = CODE + "";
   
    var rec = this.lkpArrDETAIL.find((x:any) => x.CODE === CODE);
    console.log("Checking:CODE:lkpArrGetDETAIL:", CODE,rec, this.lkpArrDETAIL )
    return rec;
    }
  
    public lkpArrGetDEPENDANT_WO_ORDER(CODE: any): any {
      // Change x.CODE below if not from SOM_TABS_CODE
      CODE = CODE + "";
      var rec = this.lkpArrDEPENDANT_WO_ORDER.find((x:any) => x.CODE === CODE);
      return rec;
      }
  ...
  public lkpArrGetASSIGNEE(CODE: any, TYPE_NAME: any): any {
    ...
    if (TYPE_NAME == "TEAM")
      ...
      var rec = this.lkpArrASSIGNEE_ROLE.find((x:any) => x.CODE === CODE);
...
@Input() public set setComponentConfig_Input(ComponentConfig: componentConfigDef) {
    if (typeof ComponentConfig !== "undefined"){
      if (this.paramConfig.DEBUG_FLAG) console.log("ComponentConfig:DspTemplateDetailGridComponent:", ComponentConfig);
  ...
  public saveOrders(object:any, masterSaved:any){
    ...
    for (var i = 0; i < object.grid.data.total ; i++){
        ...
        workOrder.SEQUENCE_NAME =  object.grid.data.data[i].SEQUENCE_NAME ;
        workOrder.REJECT_SEQ =  object.grid.data.data[i].REJECT_SEQ ;
        workOrder.MODIFY_SEQ =  object.grid.data.data[i].MODIFY_SEQ ;
        workOrder.APPROVE_SEQ =  object.grid.data.data[i].APPROVE_SEQ ;
