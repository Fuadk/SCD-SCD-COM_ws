...
export class DspWorkOrdersMyComponent implements OnInit {
  ...
  constructor(public responsive: BreakpointObserver ,public starServices: starServices) {
    ...
    if (this.paramConfig.DEBUG_FLAG) console.log(this.paramConfig)
    var lkpArrASSIGNEEDef=	{"statment":"SELECT USERNAME  CODE, FULLNAME CODETEXT_LANG, DEPT FROM ADM_USER_INFORMATION  where TEAM = '" + this.starServices.sessionParams.USER_INFO.TEAM + "' ",
    "lkpArrName":"lkpArrASSIGNEE"};
    this.lookupArrDef.push(lkpArrASSIGNEEDef);
    if (this.starServices.sessionParams.USER_INFO.LEADER == "1")
...
private Body:any =[];
  ...
  //  this.grid_DSP_WORK_ORDERS = new workOrders(); 
   ...
   this.DSP_WORK_ORDERSGridConfig.showHideGridFlag = false;
  ...
  public  executeQuery( form: any ): void {
    ...
    var whereClause = "";
    if (this.starServices.sessionParams.USER_INFO.LEADER == "1"){
      whereClause = "(assignee_type = 'TEAM' and assignee = '" + this.starServices.sessionParams.USER_INFO.TEAM + "')" ;
      ...
      {
        whereClause = whereClause + " or ( assignee_type = 'PERSON' and assignee in (select username from adm_user_information where team = '" + this.starServices.sessionParams.USER_INFO.TEAM + "' )) "
      }
    }
    else{
      whereClause = "  assignee_type = 'PERSON' and assignee = '" + this.starServices.sessionParams.USER_INFO.USERNAME + "' ";
    }
