...
const createFormGroup = (dataItem) => new FormGroup({
  ...
  'BATCH_LOG_ID': new FormControl(dataItem.BATCH_LOG_ID), 
...
private Body:any = [];
  ...
  public action = "";
  private fieldsRequiredMsg = "Please select a Process Flow and enter the required fields.";
  ...
  public async ngOnInit() {
    ...
    if (this.starServices.sessionParams.USER_INFO.GROUPNAME != "SYSADM"){
      ...
      this.formInitialValues.ASSIGNEE = this.hideOthers ? "%" : this.starServices.sessionParams.USER_INFO.TEAM;
    }
    ...
    var lkpArrASSIGNEEDef = {
      "statment": "SELECT TEAM  CODE, FULLNAME CODETEXT_LANG FROM ADM_TEAM WHERE DEPT = '"
        + this.starServices.sessionParams.USER_INFO.DEPT + "'  and DIVS = '" + this.starServices.sessionParams.USER_INFO.DIVS + "' ",
  ...
  filterWorkOrder(wo) {
    if (wo["ASSIGNEE_TYPE"] == "TEAM") {
      if (this.starServices.sessionParams.USER_INFO.MANAGER == 1 && this.starServices.sessionParams.USER_INFO.DEPT == wo["ASSIGNEE"]) return true
      // if (wo["DEPT"] ==  this.starServices.sessionParams.USER_INFO.DEPT) return true;
    }

    return wo["ASSIGNEE_TYPE"] == "MGR" && this.starServices.sessionParams.USER_INFO.MANAGER == 1
  }
  public currentData;
  public reShowMultiStepForm(){
    this.getOrderOutput.emit(this.currentData)
  }
  getOrder(id) {
    this.Body = []
    this.addToBody({
  ...
  public callBackFunction(data:any) {
    ...
    this.starServices.post(this, Page, Body).subscribe(result => {
      if (this.paramConfig.DEBUG_FLAG) console.log("result.data[0].data: ", result.data[0].data)
      this.templateDetail = result.data[0].data;
      if (this.paramConfig.DEBUG_FLAG) console.log("this.templateDetail:", this.templateDetail);
    },
      err => {
...
this.form.get('ASSIGNEE_TYPE').valueChanges.subscribe(val => {
  ...
  public savemultiStepFormCompletedHandler(DSP_MULTISTEP) {
    if (this.paramConfig.DEBUG_FLAG) console.log("test1:savemultiStepFormCompletedHandler :", this.fieldsFormSave, " DSP_DYNAMIC_RW:", DSP_MULTISTEP);
  ...
  @Input() public set setComponentConfig_Input(ComponentConfig: componentConfigDef) {
    ...
    if (ComponentConfig.masterParams != null) {
      ...
      if (ComponentConfig.masterParams.orderField) {
        this.form.reset({
          "ORDER_FIELDS": ComponentConfig.masterParams.orderField
        })
        this.submitted = true;
      }
      if (ComponentConfig.masterParams.WOStatus) {
        this.changeWOStatus(ComponentConfig.masterParams)
      }
      if (ComponentConfig.masterParams.hideOthers) {
    ...
    if (typeof ComponentConfig !== "undefined") {
      ...
      if (typeof ComponentConfig.masterParams !== "undefined") {
        if (this.paramConfig.DEBUG_FLAG) console.log("submitted:", ComponentConfig.masterParams);
          if (typeof ComponentConfig.masterParams.submitted !== "undefined") {
            this.submitted  = ComponentConfig.masterParams.submitted;
          }
      }
    }
    }
  ...
  changeWOStatus(masterparam) {
    let status = masterparam.WOStatus;
    this.form.patchValue({
      NOTES: masterparam.NoteValue
    })
    let orderStatus = 20; 
    if (this.lastWO == this.workOrder["WO_ORDER_NO"]){
        orderStatus = status; 
    }
    ...
    this.addToBody(
      ...
      "ORDER_NO": this.workOrder["ORDER_NO"],
      "TEMPLATE_NAME" : this.workOrder.TEMPLATE_NAME,
      "SEQUENCE_NAME" : this.workOrder.SEQUENCE_NAME,
      "ORDER_FIELDS" : this.form.value['ORDER_FIELDS'],
    ...
    let templateDetail;
    let i=0;
    while (i<this.templateDetail.length){
      console.log("changeWOStatus:SEQUENCE_NAME:", this.templateDetail[i].SEQUENCE_NAME, this.workOrder['SEQUENCE_NAME'])
      if (this.templateDetail[i].SEQUENCE_NAME == this.workOrder['SEQUENCE_NAME']){
        templateDetail = this.templateDetail[i];
        break;
      }
      i++;
    }
    console.log("changeWOStatus:this.workOrder:", this.workOrder, "templateDetail:", templateDetail)
    let sqlStmt = "UPDATE  DSP_WORK_ORDERS SET WO_STATUS = 40 WHERE ORDER_NO = '" + this.workOrder.ORDER_NO + "' ";
    let whereClause = "";

    if (typeof templateDetail !== "undefined") {
      console.log("changeWOStatus:status:", status,"templateDetail:",  templateDetail)
        if (status == this.paramConfig.MODIFIED){        // Handle Modified
          if (templateDetail.MODIFY_SEQ == -1){ // 	Goto End 
            whereClause = " AND TEMPLATE_ORDER > " + this.workOrder.TEMPLATE_ORDER;
            orderStatus = this.paramConfig.MODIFIED;
          }
          else if (templateDetail.MODIFY_SEQ != ""){
            let i = 0;
            let modifytSeq = templateDetail.MODIFY_SEQ;
            let templateDetailModify;
            while (i<this.templateDetail.length){
              console.log("changeWOStatus:SEQUENCE_NAME:", this.templateDetail[i].SEQUENCE_NAME, this.workOrder['SEQUENCE_NAME'], modifytSeq)
              if (this.templateDetail[i].SEQUENCE_NAME == modifytSeq){
                templateDetailModify = this.templateDetail[i];
                break;
              }
              i++;
            }
            if (typeof templateDetailModify !== "undefined") {
              sqlStmt = "UPDATE  DSP_WORK_ORDERS SET WO_STATUS = 10 WHERE ORDER_NO = '" + this.workOrder.ORDER_NO + "' ";
              whereClause = " AND TEMPLATE_ORDER = " + templateDetailModify.TEMPLATE_ORDER ;
            }
          }
      }
      if (status == this.paramConfig.REJECTED){        // Handle Rejected
        if (templateDetail.REJECT_SEQ == -1 || masterparam.ApprovalType == "FINAL"){         //	Goto End 
          whereClause = " AND TEMPLATE_ORDER > " + this.workOrder.TEMPLATE_ORDER;
          orderStatus = this.paramConfig.REJECTED;
        }
        else if (templateDetail.REJECT_SEQ != ""){
          let i = 0;
          let rejectSeq = templateDetail.REJECT_SEQ;
          let templateDetailReject;
          while (i<this.templateDetail.length){
            console.log("changeWOStatus:SEQUENCE_NAME:", this.templateDetail[i].SEQUENCE_NAME, this.workOrder['SEQUENCE_NAME'])
            if (this.templateDetail[i].SEQUENCE_NAME == rejectSeq){
              templateDetailReject = this.templateDetail[i];
              break;
            }
            i++;
          }
          if (typeof templateDetailReject !== "undefined") {
            whereClause = " AND TEMPLATE_ORDER > " + this.workOrder.TEMPLATE_ORDER + " AND TEMPLATE_ORDER < " + templateDetailReject.TEMPLATE_ORDER ;
          }
        }
      }
      else if (status == this.paramConfig.APPROVED){        // Handle Approved
        if (templateDetail.APPROVE_SEQ == -1 || masterparam.ApprovalType == "FINAL"){              //	Goto End 
          whereClause = " AND TEMPLATE_ORDER > " + this.workOrder.TEMPLATE_ORDER;
          orderStatus = this.paramConfig.APPROVED;
        }
        else if (templateDetail.APPROVE_SEQ != ""){
          let i = 0;
          let approveSeq = templateDetail.APPROVE_SEQ;
          let templateDetailApprove;
          while (i<this.templateDetail.length){
            console.log("changeWOStatus:SEQUENCE_NAME:", this.templateDetail[i].SEQUENCE_NAME, this.workOrder['SEQUENCE_NAME'])
            if (this.templateDetail[i].SEQUENCE_NAME == approveSeq){
              templateDetailApprove = this.templateDetail[i];
              break;
            }
            i++;
          }
          console.log("changeWOStatus:templateDetailApprove:", templateDetailApprove);
          if (typeof templateDetailApprove !== "undefined") {
            whereClause = " AND TEMPLATE_ORDER > " + this.workOrder.TEMPLATE_ORDER + " AND TEMPLATE_ORDER < " + templateDetailApprove.TEMPLATE_ORDER ;
          }
        }
      }

    }
    if (status == this.paramConfig.APPROVED){  
           let sqlStmt2 = "UPDATE  DSP_WORK_ORDERS SET WO_STATUS = 10 WHERE ORDER_NO = '" + this.workOrder.ORDER_NO + "' "
                        + " AND TEMPLATE_ORDER > " + this.workOrder.TEMPLATE_ORDER  + " AND WO_STATUS = '" + this.paramConfig.MODIFIED + "' " ;
          this.addToBody(
            {
            "_QUERY": "GET_STMT",
            "_STMT": sqlStmt2,
    ...
    console.log("changeWOStatus:sqlStmt:",sqlStmt, "whereClause:", whereClause)
    if (whereClause != ""){
  ...
  public colorCode() {
    ...
    let code = this.form.value['ORDER_STATUS'];
    //console.log("colorCode:", code)
