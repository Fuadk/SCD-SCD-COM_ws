...
export class AiAiAiFormFormComponent {
  ...
  constructor(public router: Router, public intl: IntlService, public responsive: BreakpointObserver, private starNotify: StarNotifyService, private starlib1: Starlib1, public starServices: starServices) {
    ...
    this.formInitialValues['STANDARD_SHAPES'] = true;
...
=== OUTPUT RULES ===
  ...
  getJsonArr_del(answer, KeyToGet) {
...
Analyze the attached industrial diagram image.
Identify every visible object and return ONLY JSON.
Rules:
1. Detect:
   - pipes
   - valves
...
2. For every detected object return:
{
  "id":"",
  "type":"",
  "bbox":{
...
3. Coordinates must be image pixel coordinates.
4. Return all pipes separately.
5. Pipes should be represented as line segments:
{
  "id":"",
  "type":"pipe",
  "points":[
...
6. Return valves separately.
7. Return motors separately.
8. Return pumps separately.
9. Return arrows separately.
...
also provide connection points for example:
{
  "id":"pump1",
  "type":"pump",
...
Return JSON only.
  ...
  async submit() {
    ...
    if (!this.simulate && this.form.value['question'] == "" && this.imgData == "") {
    ...
    let q_prefix = "This is a new requirement. I need you to generate a JSON configuration for a Kendo UI diagram following the exact structure below. "
      + ` 

      Normalize coordinates.

After all objects are detected:

1. Compute diagram bounding box.

2. Translate all objects so the diagram
starts at x=0,y=0.

3. Add a configurable margin of 50px.
...
4. Return normalized coordinates.
        ...
        2. Each shape type gets its own full definition 
...
For every object returned by the vision model,
there must be exactly one ShapeOption.

ShapeOption count MUST equal
equipment count + pipe count + label count + arrow count.
And make pipes visible, do not make height or width as zero, 
select suitable value according to json provided, and make FILL same for all and its color similar to the other shapes.
Ignore decorative text.

          CRITICAL if shape coordinates are provided:
          DO NOT calculate new positions.
          DO NOT use grid, tree, layered, force, or auto-layout.
          USE EXACT coordinates from the detected object bbox.
          ShapeOption.x = bbox.x
          ShapeOption.y = bbox.y
          ShapeOption.width = bbox.width
          ShapeOption.height = bbox.height

          Please generate the top-level ShapeOptions[] without definition for now.
    + `
