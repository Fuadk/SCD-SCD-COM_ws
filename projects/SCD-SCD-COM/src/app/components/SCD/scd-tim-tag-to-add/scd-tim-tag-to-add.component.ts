...
import { scdtagInModelScdTimTagToAdd , componentConfigDef} from '@modeldir/model';


 const createFormGroup = (dataItem:any) => new FormGroup({
'TAG_IN_MODEL_ID' : new FormControl(dataItem.TAG_IN_MODEL_ID  , ) ,
'APPLICATION_ID' : new FormControl(dataItem.APPLICATION_ID  ,   Validators.required ) ,
'TAG_ID' : new FormControl(dataItem.TAG_ID  ,   Validators.required ) ,
...
@Component({
  selector: 'app-scd-tim-tag-to-add',
  encapsulation: ViewEncapsulation.None,
  templateUrl: './scd-tim-tag-to-add.component.html',
  styleUrls: ['./scd-tim-tag-to-add.component.scss'],
  standalone: false
})


export class ScdTagInModelScdTimTagToAddFormComponent {
  public title =  this.starServices.getNLS([],"SCD_TIM_TAG_TO_ADD.scdtagInModelScdTimTagToAdd.component_title","Tag to Add");
  public compTitleMsg =  "SCD_TIM_TAG_TO_ADD.scdtagInModelScdTimTagToAdd";
  public routineName = "ScdTagInModelScdTimTagToAddForm";
  private insertCMD = "INSERT_SCD_TAG_IN_MODEL";
  private updateCMD = "UPDATE_SCD_TAG_IN_MODEL";
  private deleteCMD =   "DELETE_SCD_TAG_IN_MODEL";
  private getCMD = "GET_SCD_TAG_IN_MODEL_QUERY";
  ...
  public primarKeyReadOnlyArr = {isTAG_IN_MODEL_IDreadOnly : false , isAPPLICATION_IDreadOnly : false , isTAG_IDreadOnly : false};  
  public paramConfig;
  ...
  public compSelector = 'app-scd-tim-tag-to-add';
  public PK_AUTO = 'TAG_IN_MODEL_ID';
  public customerFacing = false;
  public FormStepsArr = [] ;
public labelTAG_IN_MODEL_IDTop=false;
public labelTAG_IN_MODEL_IDVisible=true;
...
public labelADD_TAGTop=false;
public labelADD_TAGVisible=true;
public labelREMOVE_TAGTop=false;
public labelREMOVE_TAGVisible=true;
public labelREMOVE_ALLTop=false;
public labelREMOVE_ALLVisible=true;

public visibleTAG_IN_MODEL_ID = false;
public visibleAPPLICATION_ID = false;
public visibleTAG_ID = false;
public visibleTAG_NAME = true;
public visibleTAG_TYPE = true;
public visibleADD_TAG = true;
public visibleREMOVE_TAG = true;
public visibleREMOVE_ALL = true;

public disableTAG_IN_MODEL_ID = false;
public disableAPPLICATION_ID = false;
public disableTAG_ID = false;
public disableTAG_NAME = false;
public disableTAG_TYPE = false;
public disableADD_TAG = false;
public disableREMOVE_TAG = false;
public disableREMOVE_ALL = false;

public variableADD_TAG;
public variableREMOVE_TAG;
public variableREMOVE_ALL;
 ...
 // Subscribing the event.
  ...
  private formInitialValues:any =   new scdtagInModelScdTimTagToAdd();   
    @Input() public set detail_Input(form: any) {
      ...
      /*
    if (this.paramConfig.DEBUG_FLAG) console.log('detail_Input ScdTagInModelScdTimTagToAddForm form.APPLICATION_ID :' + form.APPLICATION_ID);
  ...
  async  enterQuery (form : any){
    ...
    async callBackPost_Insert(NewVal:any) {
      ...
      this.Comp_Config.masterKeyArr =  [NewVal['TAG_IN_MODEL_ID']];
      this.Comp_Config.masterKeyNameArr =  ["TAG_IN_MODEL_ID"];
...
this.lookupArrDef =[	{"statment":"SELECT APPLICATION_ID CODE, APPLICATION_NAME CODETEXT_LANG  FROM SCD_APPLICATION  order by CODETEXT_LANG ",
	...
	{"statment":"SELECT TAG_ID CODE, TAG_NAME CODETEXT_LANG  FROM SCD_TAG  order by CODETEXT_LANG ",
			"lkpArrName":"lkpArrTAG_ID"}];
...
public lkpArrGetTAG_ID(CODE: any): any {
var rec = this.lkpArrTAG_ID.find((x:any) => x.CODE === CODE);
return rec;
}

onChanges(): void {
this.form.get('TAG_IN_MODEL_ID').valueChanges.subscribe(val => {
});
...
public printScreen(){
  ...
  public handleComponentConfig(ComponentConfig:any) {
    if (typeof ComponentConfig !== "undefined") {
      if (this.paramConfig.DEBUG_FLAG) console.log("ScdTagInModelScdTimTagToAddForm ComponentConfig:", {...ComponentConfig});
...
async WHEN_VALIDATE_ITEM_TAG_IN_MODEL_ID(value) {

 this.FORM_TRIGGER_FAILURE = false ; 
 if (typeof this.form.controls['TAG_IN_MODEL_ID'] != "undefined" ) 
      this.form.controls['TAG_IN_MODEL_ID'].setErrors({invalid: true}); 
 // Code goes here 
 ...
 if (typeof this.form.controls['TAG_IN_MODEL_ID'] != "undefined" ) 
     this.form.get('TAG_IN_MODEL_ID').updateValueAndValidity();
 this.form.updateValueAndValidity(); 
 }

 async ON_CLICK_TAG_IN_MODEL_ID(event){
...
async WHEN_VALIDATE_ITEM_ADD_TAG(value) {

 this.FORM_TRIGGER_FAILURE = false ; 
 if (typeof this.form.controls['ADD_TAG'] != "undefined" ) 
      this.form.controls['ADD_TAG'].setErrors({invalid: true}); 
 // Code goes here 
 ...
 if (typeof this.form.controls['ADD_TAG'] != "undefined" ) 
     this.form.get('ADD_TAG').updateValueAndValidity();
 this.form.updateValueAndValidity(); 
 }

 async ON_CLICK_ADD_TAG(event){

}

async WHEN_VALIDATE_ITEM_REMOVE_TAG(value) {

 this.FORM_TRIGGER_FAILURE = false ; 
 if (typeof this.form.controls['REMOVE_TAG'] != "undefined" ) 
      this.form.controls['REMOVE_TAG'].setErrors({invalid: true}); 
 // Code goes here 
 ...
 if (typeof this.form.controls['REMOVE_TAG'] != "undefined" ) 
     this.form.get('REMOVE_TAG').updateValueAndValidity();
 this.form.updateValueAndValidity(); 
 }

 async ON_CLICK_REMOVE_TAG(event){

}

async WHEN_VALIDATE_ITEM_REMOVE_ALL(value) {

 this.FORM_TRIGGER_FAILURE = false ; 
 if (typeof this.form.controls['REMOVE_ALL'] != "undefined" ) 
      this.form.controls['REMOVE_ALL'].setErrors({invalid: true}); 
 // Code goes here 
 ...
 if (typeof this.form.controls['REMOVE_ALL'] != "undefined" ) 
     this.form.get('REMOVE_ALL').updateValueAndValidity();
 this.form.updateValueAndValidity(); 
 }

 async ON_CLICK_REMOVE_ALL(event){

}
 
 async onChange_TAG_IN_MODEL_ID(event:any) { 
 var value = event.target.value; 
 if ((value == null) || (value == '')) 	
 	return;  
    this.FORM_TRIGGER_FAILURE = false;	
 await   this.WHEN_VALIDATE_ITEM_TAG_IN_MODEL_ID(value); if ( this.FORM_TRIGGER_FAILURE) return; 
 this.formValidationChangedOutput.emit(this.form.valid); 
 ...
 async onValueChange_TAG_ID(value) { 
  this.FORM_TRIGGER_FAILURE = false;	
 ...
 async onValueChange_ADD_TAG(value) { 
  this.FORM_TRIGGER_FAILURE = false;	
 await this.WHEN_VALIDATE_ITEM_ADD_TAG(value); if ( this.FORM_TRIGGER_FAILURE) return; 
 this.formValidationChangedOutput.emit(this.form.valid); 
  
  } 
 async onValueChange_REMOVE_TAG(value) { 
  this.FORM_TRIGGER_FAILURE = false;	
 await this.WHEN_VALIDATE_ITEM_REMOVE_TAG(value); if ( this.FORM_TRIGGER_FAILURE) return; 
 this.formValidationChangedOutput.emit(this.form.valid); 
  
  } 
 async onValueChange_REMOVE_ALL(value) { 
  this.FORM_TRIGGER_FAILURE = false;	
 await this.WHEN_VALIDATE_ITEM_REMOVE_ALL(value); if ( this.FORM_TRIGGER_FAILURE) return; 
 this.formValidationChangedOutput.emit(this.form.valid); 
