...
import { scdtagLabelGeneralScdTlgTagLabelGeneral , componentConfigDef} from '@modeldir/model';


 const createFormGroup = (dataItem:any) => new FormGroup({
'TAG_LABEL_GENERAL_ID' : new FormControl(dataItem.TAG_LABEL_GENERAL_ID  , ) ,
'SHAPE_ID' : new FormControl(dataItem.SHAPE_ID  ,   Validators.required ) ,
'TAG_NAME' : new FormControl(dataItem.TAG_NAME  , ) ,
'TAG_PRIORITY' : new FormControl(dataItem.TAG_PRIORITY  , ) ,
...
@Component({
  selector: 'app-scd-tlg-tag-label-general',
  encapsulation: ViewEncapsulation.None,
  templateUrl: './scd-tlg-tag-label-general.component.html',
  styleUrls: ['./scd-tlg-tag-label-general.component.scss'],
  standalone: false
})


export class ScdTagLabelGeneralScdTlgTagLabelGeneralFormdivsComponent {
  public title =  this.starServices.getNLS([],"SCD_TLG_TAG_LABEL_GENERAL.scdtagLabelGeneralScdTlgTagLabelGeneral.component_title","Tag Label General");
  public compTitleMsg =  "SCD_TLG_TAG_LABEL_GENERAL.scdtagLabelGeneralScdTlgTagLabelGeneral";
  public routineName = "ScdTagLabelGeneralScdTlgTagLabelGeneralFormdivs";
  private insertCMD = "INSERT_SCD_TAG_LABEL_GENERAL";
  private updateCMD = "UPDATE_SCD_TAG_LABEL_GENERAL";
  private deleteCMD =   "DELETE_SCD_TAG_LABEL_GENERAL";
  private getCMD = "GET_SCD_TAG_LABEL_GENERAL_QUERY";
  ...
  public primarKeyReadOnlyArr = {isTAG_LABEL_GENERAL_IDreadOnly : false , isSHAPE_IDreadOnly : false};  
  public paramConfig;
  ...
  public compSelector = 'app-scd-tlg-tag-label-general';
  public PK_AUTO = 'TAG_LABEL_GENERAL_ID';
  public customerFacing = false;
  public FormStepsArr = [{"CODE":"","CODETEXT_LANG":"","visible":true},{"CODE":"1","CODETEXT_LANG":"Tag","visible":true},{"CODE":"2","CODETEXT_LANG":"Justification","visible":true}] ;
public labelTAG_LABEL_GENERAL_IDTop=false;
public labelTAG_LABEL_GENERAL_IDVisible=true;
...
public labelTAG_SELECTTop=false;
public labelTAG_SELECTVisible=true;
public labelTAG_PRIORITYTop=false;
public labelTAG_PRIORITYVisible=true;
...
public variableTAG_SELECT;
 ...
 // Subscribing the event.
  ...
  private formInitialValues:any =   new scdtagLabelGeneralScdTlgTagLabelGeneral();   
    @Input() public set detail_Input(form: any) {
      ...
      /*
    if (this.paramConfig.DEBUG_FLAG) console.log('detail_Input ScdTagLabelGeneralScdTlgTagLabelGeneralFormdivs form.SHAPE_ID :' + form.SHAPE_ID);
  ...
  async  enterQuery (form : any){
    ...
    async callBackPost_Insert(NewVal:any) {
      ...
      this.Comp_Config.masterKeyArr =  [NewVal['TAG_LABEL_GENERAL_ID']];
      this.Comp_Config.masterKeyNameArr =  ["TAG_LABEL_GENERAL_ID"];
...
this.lookupArrDef =[	{"statment":"SELECT SHAPE_ID CODE, NAME CODETEXT_LANG  FROM SCD_SHAPE  order by CODETEXT_LANG ",
	...
	{"statment":"SELECT CODE, CODETEXT_LANG , PARTCODE FROM SOM_TABS_CODES WHERE CODENAME = \"TAG_PRIORITY\"  and LANGUAGE_NAME = '" + this.userLang + "' order by CODETEXT_LANG ",
			"lkpArrName":"lkpArrTAG_PRIORITY"},
...
public lkpArrGetTAG_PRIORITY(CODE: any): any {
var rec = this.lkpArrTAG_PRIORITY.find((x:any) => x.CODE === CODE);
return rec;
}
...
onChanges(): void {
this.form.get('TAG_LABEL_GENERAL_ID').valueChanges.subscribe(val => {
});
...
public printScreen(){
  ...
  public handleComponentConfig(ComponentConfig:any) {
    if (typeof ComponentConfig !== "undefined") {
      if (this.paramConfig.DEBUG_FLAG) console.log("ScdTagLabelGeneralScdTlgTagLabelGeneralFormdivs ComponentConfig:", {...ComponentConfig});
...
async WHEN_VALIDATE_ITEM_TAG_LABEL_GENERAL_ID(value) {

 this.FORM_TRIGGER_FAILURE = false ; 
 if (typeof this.form.controls['TAG_LABEL_GENERAL_ID'] != "undefined" ) 
      this.form.controls['TAG_LABEL_GENERAL_ID'].setErrors({invalid: true}); 
 // Code goes here 
 ...
 if (typeof this.form.controls['TAG_LABEL_GENERAL_ID'] != "undefined" ) 
     this.form.get('TAG_LABEL_GENERAL_ID').updateValueAndValidity();
 this.form.updateValueAndValidity(); 
 }

 async ON_CLICK_TAG_LABEL_GENERAL_ID(event){
...
async WHEN_VALIDATE_ITEM_TAG_SELECT(value) {

 this.FORM_TRIGGER_FAILURE = false ; 
 if (typeof this.form.controls['TAG_SELECT'] != "undefined" ) 
      this.form.controls['TAG_SELECT'].setErrors({invalid: true}); 
 // Code goes here 
 ...
 if (typeof this.form.controls['TAG_SELECT'] != "undefined" ) 
     this.form.get('TAG_SELECT').updateValueAndValidity();
 this.form.updateValueAndValidity(); 
 }

 async ON_CLICK_TAG_SELECT(event){

}

async WHEN_VALIDATE_ITEM_TAG_PRIORITY(value) {

 this.FORM_TRIGGER_FAILURE = false ; 
 if (typeof this.form.controls['TAG_PRIORITY'] != "undefined" ) 
      this.form.controls['TAG_PRIORITY'].setErrors({invalid: true}); 
 // Code goes here 
 ...
 if (typeof this.form.controls['TAG_PRIORITY'] != "undefined" ) 
     this.form.get('TAG_PRIORITY').updateValueAndValidity();
 this.form.updateValueAndValidity(); 
 }

 async ON_CLICK_TAG_PRIORITY(event){
...
async WHEN_VALIDATE_ITEM_JUSTIFICATION(value) {
 ...
 async onChange_TAG_LABEL_GENERAL_ID(event:any) { 
 var value = event.target.value; 
 if ((value == null) || (value == '')) 	
 	return;  
    this.FORM_TRIGGER_FAILURE = false;	
 await   this.WHEN_VALIDATE_ITEM_TAG_LABEL_GENERAL_ID(value); if ( this.FORM_TRIGGER_FAILURE) return; 
 this.formValidationChangedOutput.emit(this.form.valid); 
 ...
 async onValueChange_TAG_SELECT(value) { 
  this.FORM_TRIGGER_FAILURE = false;	
 await this.WHEN_VALIDATE_ITEM_TAG_SELECT(value); if ( this.FORM_TRIGGER_FAILURE) return; 
 this.formValidationChangedOutput.emit(this.form.valid); 
  
  } 
 async onValueChange_TAG_PRIORITY(value) { 
  this.FORM_TRIGGER_FAILURE = false;	
 await this.WHEN_VALIDATE_ITEM_TAG_PRIORITY(value); if ( this.FORM_TRIGGER_FAILURE) return; 
 this.formValidationChangedOutput.emit(this.form.valid); 
