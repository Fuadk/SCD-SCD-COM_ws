...
import { scdmacroScdAmMacroDetails , componentConfigDef} from '@modeldir/model';


 const createFormGroup = (dataItem:any) => new FormGroup({
'MACRO_ID' : new FormControl(dataItem.MACRO_ID  , ) ,
'APPLICATION_ID' : new FormControl(dataItem.APPLICATION_ID  ,   Validators.required ) ,
'MACRO_NAME' : new FormControl(dataItem.MACRO_NAME  , ) ,
'MACRO_TEXT' : new FormControl(dataItem.MACRO_TEXT  , ) 
});

declare function getParamConfig():any;
@Component({
  selector: 'app-scd-am-macro-details',
  encapsulation: ViewEncapsulation.None,
  templateUrl: './scd-am-macro-details.component.html',
  styleUrls: ['./scd-am-macro-details.component.scss'],
  standalone: false
})


export class ScdMacroScdAmMacroDetailsFormComponent {
  public title =  this.starServices.getNLS([],"SCD_AM_MACRO_DETAILS.scdmacroScdAmMacroDetails.component_title","Macro Details");
  public compTitleMsg =  "SCD_AM_MACRO_DETAILS.scdmacroScdAmMacroDetails";
  public routineName = "ScdMacroScdAmMacroDetailsForm";
  private insertCMD = "INSERT_SCD_MACRO";
  private updateCMD = "UPDATE_SCD_MACRO";
  private deleteCMD =   "DELETE_SCD_MACRO";
  private getCMD = "GET_SCD_MACRO_QUERY";
  ...
  public primarKeyReadOnlyArr = {isMACRO_IDreadOnly : false , isAPPLICATION_IDreadOnly : false};  
  public paramConfig;
  ...
  public compSelector = 'app-scd-am-macro-details';
  public PK_AUTO = 'MACRO_ID';
  public customerFacing = false;
  public FormStepsArr = [] ;
public labelMACRO_IDTop=false;
public labelMACRO_IDVisible=true;
public labelAPPLICATION_IDTop=false;
public labelAPPLICATION_IDVisible=true;
public labelMACRO_NAMETop=false;
public labelMACRO_NAMEVisible=true;
public labelMACRO_TEXTTop=false;
public labelMACRO_TEXTVisible=true;

public visibleMACRO_ID = false;
public visibleAPPLICATION_ID = false;
public visibleMACRO_NAME = true;
public visibleMACRO_TEXT = true;

public disableMACRO_ID = false;
public disableAPPLICATION_ID = false;
public disableMACRO_NAME = false;
public disableMACRO_TEXT = false;
 ...
 // Subscribing the event.
  ...
  private formInitialValues:any =   new scdmacroScdAmMacroDetails();   
    @Input() public set detail_Input(form: any) {
      ...
      /*
    if (this.paramConfig.DEBUG_FLAG) console.log('detail_Input ScdMacroScdAmMacroDetailsForm form.APPLICATION_ID :' + form.APPLICATION_ID);
  ...
  async  enterQuery (form : any){
    ...
    async callBackPost_Insert(NewVal:any) {
      ...
      this.Comp_Config.masterKeyArr =  [NewVal['MACRO_ID']];
      this.Comp_Config.masterKeyNameArr =  ["MACRO_ID"];
...
onChanges(): void {
this.form.get('MACRO_ID').valueChanges.subscribe(val => {
});
this.form.get('MACRO_NAME').valueChanges.subscribe(val => {
});
}
...
public printScreen(){
  ...
  public handleComponentConfig(ComponentConfig:any) {
    if (typeof ComponentConfig !== "undefined") {
      if (this.paramConfig.DEBUG_FLAG) console.log("ScdMacroScdAmMacroDetailsForm ComponentConfig:", {...ComponentConfig});
...
async WHEN_VALIDATE_ITEM_MACRO_ID(value) {

 this.FORM_TRIGGER_FAILURE = false ; 
 if (typeof this.form.controls['MACRO_ID'] != "undefined" ) 
      this.form.controls['MACRO_ID'].setErrors({invalid: true}); 
 // Code goes here 
 ...
 if (typeof this.form.controls['MACRO_ID'] != "undefined" ) 
     this.form.get('MACRO_ID').updateValueAndValidity();
 this.form.updateValueAndValidity(); 
 }

 async ON_CLICK_MACRO_ID(event){
...
async WHEN_VALIDATE_ITEM_MACRO_NAME(value) {

 this.FORM_TRIGGER_FAILURE = false ; 
 if (typeof this.form.controls['MACRO_NAME'] != "undefined" ) 
      this.form.controls['MACRO_NAME'].setErrors({invalid: true}); 
 // Code goes here 
 ...
 if (typeof this.form.controls['MACRO_NAME'] != "undefined" ) 
     this.form.get('MACRO_NAME').updateValueAndValidity();
 this.form.updateValueAndValidity(); 
 }

 async ON_CLICK_MACRO_NAME(event){

}

async WHEN_VALIDATE_ITEM_MACRO_TEXT(value) {

 this.FORM_TRIGGER_FAILURE = false ; 
 if (typeof this.form.controls['MACRO_TEXT'] != "undefined" ) 
      this.form.controls['MACRO_TEXT'].setErrors({invalid: true}); 
 // Code goes here 
 ...
 if (typeof this.form.controls['MACRO_TEXT'] != "undefined" ) 
     this.form.get('MACRO_TEXT').updateValueAndValidity();
 this.form.updateValueAndValidity(); 
 }

 async ON_CLICK_MACRO_TEXT(event){

}
 
 async onChange_MACRO_ID(event:any) { 
 var value = event.target.value; 
 if ((value == null) || (value == '')) 	
 	return;  
    this.FORM_TRIGGER_FAILURE = false;	
 await   this.WHEN_VALIDATE_ITEM_MACRO_ID(value); if ( this.FORM_TRIGGER_FAILURE) return; 
 this.formValidationChangedOutput.emit(this.form.valid); 
 ...
 async onChange_MACRO_NAME(event:any) { 
 var value = event.target.value; 
 if ((value == null) || (value == '')) 	
 	return;  
    this.FORM_TRIGGER_FAILURE = false;	
 await   this.WHEN_VALIDATE_ITEM_MACRO_NAME(value); if ( this.FORM_TRIGGER_FAILURE) return; 
 this.formValidationChangedOutput.emit(this.form.valid); 
  
 } 
 async onValueChange_MACRO_TEXT(value) { 
  this.FORM_TRIGGER_FAILURE = false;	
 await this.WHEN_VALIDATE_ITEM_MACRO_TEXT(value); if ( this.FORM_TRIGGER_FAILURE) return; 
 this.formValidationChangedOutput.emit(this.form.valid); 
