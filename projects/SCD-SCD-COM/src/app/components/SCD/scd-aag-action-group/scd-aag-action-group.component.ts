...
import { scdactionGroupScdAagActionGroup , componentConfigDef} from '@modeldir/model';


 const createFormGroup = (dataItem:any) => new FormGroup({
'ACTION_GROUP_ID' : new FormControl(dataItem.ACTION_GROUP_ID  , ) ,
...
'ACTION_GROUP' : new FormControl(dataItem.ACTION_GROUP  , ) 
});

declare function getParamConfig():any;
@Component({
  selector: 'app-scd-aag-action-group',
  encapsulation: ViewEncapsulation.None,
  templateUrl: './scd-aag-action-group.component.html',
  styleUrls: ['./scd-aag-action-group.component.scss'],
  standalone: false
})


export class ScdActionGroupScdAagActionGroupFormComponent {
  public title =  this.starServices.getNLS([],"SCD_AAG_ACTION_GROUP.scdactionGroupScdAagActionGroup.component_title","Action Group");
  public compTitleMsg =  "SCD_AAG_ACTION_GROUP.scdactionGroupScdAagActionGroup";
  public routineName = "ScdActionGroupScdAagActionGroupForm";
  private insertCMD = "INSERT_SCD_ACTION_GROUP";
  private updateCMD = "UPDATE_SCD_ACTION_GROUP";
  private deleteCMD =   "DELETE_SCD_ACTION_GROUP";
  private getCMD = "GET_SCD_ACTION_GROUP_QUERY";
  ...
  public primarKeyReadOnlyArr = {isACTION_GROUP_IDreadOnly : false , isAPPLICATION_IDreadOnly : false};  
  public paramConfig;
  ...
  public compSelector = 'app-scd-aag-action-group';
  public PK_AUTO = 'ACTION_GROUP_ID';
  public customerFacing = false;
  public FormStepsArr = [] ;
public labelACTION_GROUP_IDTop=false;
public labelACTION_GROUP_IDVisible=true;
...
public labelGROUP_NAMETop=false;
public labelGROUP_NAMEVisible=true;
public labelACTION_GROUPTop=false;
public labelACTION_GROUPVisible=true;

public visibleACTION_GROUP_ID = false;
public visibleAPPLICATION_ID = false;
public visibleGROUP_ID = true;
public visibleGROUP_NAME = true;
public visibleACTION_GROUP = true;

public disableACTION_GROUP_ID = false;
public disableAPPLICATION_ID = false;
public disableGROUP_ID = false;
public disableGROUP_NAME = false;
public disableACTION_GROUP = false;
 ...
 // Subscribing the event.
  ...
  private formInitialValues:any =   new scdactionGroupScdAagActionGroup();   
    @Input() public set detail_Input(form: any) {
      ...
      /*
    if (this.paramConfig.DEBUG_FLAG) console.log('detail_Input ScdActionGroupScdAagActionGroupForm form.APPLICATION_ID :' + form.APPLICATION_ID);
  ...
  async  enterQuery (form : any){
    ...
    async callBackPost_Insert(NewVal:any) {
      ...
      this.Comp_Config.masterKeyArr =  [NewVal['ACTION_GROUP_ID']];
      this.Comp_Config.masterKeyNameArr =  ["ACTION_GROUP_ID"];
...
this.lookupArrDef =[	{"statment":"SELECT APPLICATION_ID CODE, APPLICATION_NAME CODETEXT_LANG  FROM SCD_APPLICATION  order by CODETEXT_LANG ",
	...
	{"statment":"SELECT CODE, CODETEXT_LANG, CODEVALUE_LANG FROM SOM_TABS_CODES WHERE CODENAME ='ACTION_GROUP' and LANGUAGE_NAME = '" + this.userLang + "' order by CODETEXT_LANG",
			"lkpArrName":"lkpArrACTION_GROUP"}];
...
public lkpArrGetACTION_GROUP(CODE: any): any {
var rec = this.lkpArrACTION_GROUP.find((x:any) => x.CODE === CODE);
return rec;
}

onChanges(): void {
this.form.get('ACTION_GROUP_ID').valueChanges.subscribe(val => {
});
this.form.get('GROUP_ID').valueChanges.subscribe(val => {
});
this.form.get('GROUP_NAME').valueChanges.subscribe(val => {
});
}
...
public printScreen(){
  ...
  public handleComponentConfig(ComponentConfig:any) {
    if (typeof ComponentConfig !== "undefined") {
      if (this.paramConfig.DEBUG_FLAG) console.log("ScdActionGroupScdAagActionGroupForm ComponentConfig:", {...ComponentConfig});
...
async WHEN_VALIDATE_ITEM_ACTION_GROUP_ID(value) {

 this.FORM_TRIGGER_FAILURE = false ; 
 if (typeof this.form.controls['ACTION_GROUP_ID'] != "undefined" ) 
      this.form.controls['ACTION_GROUP_ID'].setErrors({invalid: true}); 
 // Code goes here 
 ...
 if (typeof this.form.controls['ACTION_GROUP_ID'] != "undefined" ) 
     this.form.get('ACTION_GROUP_ID').updateValueAndValidity();
 this.form.updateValueAndValidity(); 
 }

 async ON_CLICK_ACTION_GROUP_ID(event){
...
async WHEN_VALIDATE_ITEM_GROUP_NAME(value) {

 this.FORM_TRIGGER_FAILURE = false ; 
 if (typeof this.form.controls['GROUP_NAME'] != "undefined" ) 
      this.form.controls['GROUP_NAME'].setErrors({invalid: true}); 
 // Code goes here 
 ...
 async ON_CLICK_GROUP_NAME(event){

}

async WHEN_VALIDATE_ITEM_ACTION_GROUP(value) {

 this.FORM_TRIGGER_FAILURE = false ; 
 if (typeof this.form.controls['ACTION_GROUP'] != "undefined" ) 
      this.form.controls['ACTION_GROUP'].setErrors({invalid: true}); 
 // Code goes here 
 ...
 if (typeof this.form.controls['ACTION_GROUP'] != "undefined" ) 
     this.form.get('ACTION_GROUP').updateValueAndValidity();
 this.form.updateValueAndValidity(); 
 }

 async ON_CLICK_ACTION_GROUP(event){

}
 
 async onChange_ACTION_GROUP_ID(event:any) { 
 var value = event.target.value; 
 if ((value == null) || (value == '')) 	
 	return;  
    this.FORM_TRIGGER_FAILURE = false;	
 await   this.WHEN_VALIDATE_ITEM_ACTION_GROUP_ID(value); if ( this.FORM_TRIGGER_FAILURE) return; 
 this.formValidationChangedOutput.emit(this.form.valid); 
 ...
 async onChange_GROUP_NAME(event:any) { 
 var value = event.target.value; 
 if ((value == null) || (value == '')) 	
 	return;  
    this.FORM_TRIGGER_FAILURE = false;	
 await   this.WHEN_VALIDATE_ITEM_GROUP_NAME(value); if ( this.FORM_TRIGGER_FAILURE) return; 
 this.formValidationChangedOutput.emit(this.form.valid); 
  
 } 
 async onValueChange_ACTION_GROUP(value) { 
  this.FORM_TRIGGER_FAILURE = false;	
 await this.WHEN_VALIDATE_ITEM_ACTION_GROUP(value); if ( this.FORM_TRIGGER_FAILURE) return; 
 this.formValidationChangedOutput.emit(this.form.valid); 
...
public multiselect_tree_arr = ["ACTION_GROUP"];
public AttDwnUrl = "";
