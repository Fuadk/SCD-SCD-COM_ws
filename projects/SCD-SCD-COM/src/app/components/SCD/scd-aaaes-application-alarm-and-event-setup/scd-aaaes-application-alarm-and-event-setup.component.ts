...
import { scdalarmScdAaaesApplicationAlarmAndEventSetup , componentConfigDef} from '@modeldir/model';


 const createFormGroup = (dataItem:any) => new FormGroup({
'ALARM_ID' : new FormControl(dataItem.ALARM_ID  , ) ,
'APPLICATION_ID' : new FormControl(dataItem.APPLICATION_ID  ,   Validators.required ) ,
'TYPE' : new FormControl(dataItem.TYPE  , ) ,
'NAME' : new FormControl(dataItem.NAME  , ) ,
'INPUT_TAG' : new FormControl(dataItem.INPUT_TAG  , ) 
});

declare function getParamConfig():any;
@Component({
  selector: 'app-scd-aaaes-application-alarm-and-event-setup',
  encapsulation: ViewEncapsulation.None,
  templateUrl: './scd-aaaes-application-alarm-and-event-setup.component.html',
  styleUrls: ['./scd-aaaes-application-alarm-and-event-setup.component.scss'],
  standalone: false
})


export class ScdAlarmScdAaaesApplicationAlarmAndEventSetupFormtabsComponent {
  public title =  this.starServices.getNLS([],"SCD_AAAES_APPLICATION_ALARM_AND_EVENT_SETUP.scdalarmScdAaaesApplicationAlarmAndEventSetup.component_title","Application Alarm and Event Setup");
  public compTitleMsg =  "SCD_AAAES_APPLICATION_ALARM_AND_EVENT_SETUP.scdalarmScdAaaesApplicationAlarmAndEventSetup";
  public routineName = "ScdAlarmScdAaaesApplicationAlarmAndEventSetupFormtabs";
  private insertCMD = "INSERT_SCD_ALARM";
  private updateCMD = "UPDATE_SCD_ALARM";
  private deleteCMD =   "DELETE_SCD_ALARM";
  private getCMD = "GET_SCD_ALARM_QUERY";
  ...
  public primarKeyReadOnlyArr = {isALARM_IDreadOnly : false , isAPPLICATION_IDreadOnly : false};  
  public paramConfig;
  ...
  public compSelector = 'app-scd-aaaes-application-alarm-and-event-setup';
  public PK_AUTO = 'ALARM_ID';
  public customerFacing = false;
  public FormStepsArr = [{"CODE":"","CODETEXT_LANG":""},{"CODE":"1","CODETEXT_LANG":"All Alarms"},{"CODE":"2","CODETEXT_LANG":"Messages"},{"CODE":"3","CODETEXT_LANG":"Tag Update Rates"}] ;
public labelALARM_IDTop=false;
public labelALARM_IDVisible=true;
public labelAPPLICATION_IDTop=false;
public labelAPPLICATION_IDVisible=true;
public labelTYPETop=false;
public labelTYPEVisible=true;
public labelNAMETop=false;
public labelNAMEVisible=true;
public labelINPUT_TAGTop=false;
public labelINPUT_TAGVisible=true;

public visibleALARM_ID = true;
public visibleAPPLICATION_ID = true;
public visibleTYPE = true;
public visibleNAME = true;
public visibleINPUT_TAG = true;

public disableALARM_ID = false;
public disableAPPLICATION_ID = false;
public disableTYPE = false;
public disableNAME = false;
public disableINPUT_TAG = false;
 ...
 // Subscribing the event.
  ...
  private formInitialValues:any =   new scdalarmScdAaaesApplicationAlarmAndEventSetup();   
    @Input() public set detail_Input(form: any) {
      ...
      /*
    if (this.paramConfig.DEBUG_FLAG) console.log('detail_Input ScdAlarmScdAaaesApplicationAlarmAndEventSetupFormtabs form.APPLICATION_ID :' + form.APPLICATION_ID);
  ...
  async  enterQuery (form : any){
    ...
    async callBackPost_Insert(NewVal:any) {
      ...
      this.Comp_Config.masterKeyArr =  [NewVal['ALARM_ID']];
      this.Comp_Config.masterKeyNameArr =  ["ALARM_ID"];
...
onChanges(): void {
this.form.get('ALARM_ID').valueChanges.subscribe(val => {
});
this.form.get('TYPE').valueChanges.subscribe(val => {
});
this.form.get('NAME').valueChanges.subscribe(val => {
});
this.form.get('INPUT_TAG').valueChanges.subscribe(val => {
});
}
...
public printScreen(){
  ...
  public handleComponentConfig(ComponentConfig:any) {
    if (typeof ComponentConfig !== "undefined") {
      if (this.paramConfig.DEBUG_FLAG) console.log("ScdAlarmScdAaaesApplicationAlarmAndEventSetupFormtabs ComponentConfig:", {...ComponentConfig});
...
async WHEN_VALIDATE_ITEM_ALARM_ID(value) {

 this.FORM_TRIGGER_FAILURE = false ; 
 if (typeof this.form.controls['ALARM_ID'] != "undefined" ) 
      this.form.controls['ALARM_ID'].setErrors({invalid: true}); 
 // Code goes here 
 ...
 if (typeof this.form.controls['ALARM_ID'] != "undefined" ) 
     this.form.get('ALARM_ID').updateValueAndValidity();
 this.form.updateValueAndValidity(); 
 }

 async ON_CLICK_ALARM_ID(event){
...
async WHEN_VALIDATE_ITEM_TYPE(value) {

 this.FORM_TRIGGER_FAILURE = false ; 
 if (typeof this.form.controls['TYPE'] != "undefined" ) 
...
async WHEN_VALIDATE_ITEM_INPUT_TAG(value) {

 this.FORM_TRIGGER_FAILURE = false ; 
 if (typeof this.form.controls['INPUT_TAG'] != "undefined" ) 
      this.form.controls['INPUT_TAG'].setErrors({invalid: true}); 
 // Code goes here 
 ...
 if (typeof this.form.controls['INPUT_TAG'] != "undefined" ) 
     this.form.get('INPUT_TAG').updateValueAndValidity();
 this.form.updateValueAndValidity(); 
 }

 async ON_CLICK_INPUT_TAG(event){

}
 
 async onChange_ALARM_ID(event:any) { 
 var value = event.target.value; 
 if ((value == null) || (value == '')) 	
 	return;  
    this.FORM_TRIGGER_FAILURE = false;	
 await   this.WHEN_VALIDATE_ITEM_ALARM_ID(value); if ( this.FORM_TRIGGER_FAILURE) return; 
 this.formValidationChangedOutput.emit(this.form.valid); 
 ...
 async onChange_TYPE(event:any) { 
 var value = event.target.value; 
 if ((value == null) || (value == '')) 	
 	return;  
    this.FORM_TRIGGER_FAILURE = false;	
 await   this.WHEN_VALIDATE_ITEM_TYPE(value); if ( this.FORM_TRIGGER_FAILURE) return; 
 this.formValidationChangedOutput.emit(this.form.valid); 
 ...
 async onChange_INPUT_TAG(event:any) { 
 var value = event.target.value; 
 if ((value == null) || (value == '')) 	
 	return;  
    this.FORM_TRIGGER_FAILURE = false;	
 await   this.WHEN_VALIDATE_ITEM_INPUT_TAG(value); if ( this.FORM_TRIGGER_FAILURE) return; 
 this.formValidationChangedOutput.emit(this.form.valid); 
