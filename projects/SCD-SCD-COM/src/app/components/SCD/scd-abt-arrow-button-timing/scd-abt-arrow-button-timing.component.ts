...
import { scdarrowButtonTimingScdAbtArrowButtonTiming , componentConfigDef} from '@modeldir/model';


 const createFormGroup = (dataItem:any) => new FormGroup({
'ARROW_BUTTON_TIMING_ID' : new FormControl(dataItem.ARROW_BUTTON_TIMING_ID  , ) ,
'SHAPE_ID' : new FormControl(dataItem.SHAPE_ID  ,   Validators.required ) ,
'AUTO_REPEAT_RATE' : new FormControl(dataItem.AUTO_REPEAT_RATE  , ) ,
'AUTO_REPEAT_DELAY' : new FormControl(dataItem.AUTO_REPEAT_DELAY  , ) 
});

declare function getParamConfig():any;
@Component({
  selector: 'app-scd-abt-arrow-button-timing',
  encapsulation: ViewEncapsulation.None,
  templateUrl: './scd-abt-arrow-button-timing.component.html',
  styleUrls: ['./scd-abt-arrow-button-timing.component.scss'],
  standalone: false
})


export class ScdArrowButtonTimingScdAbtArrowButtonTimingFormComponent {
  public title =  this.starServices.getNLS([],"SCD_ABT_ARROW_BUTTON_TIMING.scdarrowButtonTimingScdAbtArrowButtonTiming.component_title","Arrow Button Timing");
  public compTitleMsg =  "SCD_ABT_ARROW_BUTTON_TIMING.scdarrowButtonTimingScdAbtArrowButtonTiming";
  public routineName = "ScdArrowButtonTimingScdAbtArrowButtonTimingForm";
  private insertCMD = "INSERT_SCD_ARROW_BUTTON_TIMING";
  private updateCMD = "UPDATE_SCD_ARROW_BUTTON_TIMING";
  private deleteCMD =   "DELETE_SCD_ARROW_BUTTON_TIMING";
  private getCMD = "GET_SCD_ARROW_BUTTON_TIMING_QUERY";
  ...
  public primarKeyReadOnlyArr = {isARROW_BUTTON_TIMING_IDreadOnly : false , isSHAPE_IDreadOnly : false};  
  public paramConfig;
  ...
  public compSelector = 'app-scd-abt-arrow-button-timing';
  public PK_AUTO = 'ARROW_BUTTON_TIMING_ID';
  public customerFacing = false;
  public FormStepsArr = [] ;
public labelARROW_BUTTON_TIMING_IDTop=true;
public labelARROW_BUTTON_TIMING_IDVisible=true;
public labelSHAPE_IDTop=true;
public labelSHAPE_IDVisible=true;
public labelAUTO_REPEAT_RATETop=true;
public labelAUTO_REPEAT_RATEVisible=true;
public labelAUTO_REPEAT_DELAYTop=true;
public labelAUTO_REPEAT_DELAYVisible=true;

public visibleARROW_BUTTON_TIMING_ID = false;
public visibleSHAPE_ID = false;
public visibleAUTO_REPEAT_RATE = true;
public visibleAUTO_REPEAT_DELAY = true;

public disableARROW_BUTTON_TIMING_ID = false;
public disableSHAPE_ID = false;
public disableAUTO_REPEAT_RATE = false;
public disableAUTO_REPEAT_DELAY = false;
 ...
 // Subscribing the event.
  ...
  private formInitialValues:any =   new scdarrowButtonTimingScdAbtArrowButtonTiming();   
    @Input() public set detail_Input(form: any) {
      ...
      /*
    if (this.paramConfig.DEBUG_FLAG) console.log('detail_Input ScdArrowButtonTimingScdAbtArrowButtonTimingForm form.SHAPE_ID :' + form.SHAPE_ID);
  ...
  async  enterQuery (form : any){
    ...
    async callBackPost_Insert(NewVal:any) {
      ...
      this.Comp_Config.masterKeyArr =  [NewVal['ARROW_BUTTON_TIMING_ID']];
      this.Comp_Config.masterKeyNameArr =  ["ARROW_BUTTON_TIMING_ID"];
...
this.lookupArrDef =[	{"statment":"SELECT SHAPE_ID CODE, NAME CODETEXT_LANG  FROM SCD_SHAPE  order by CODETEXT_LANG ",
	...
	{"statment":"SELECT CODE, CODETEXT_LANG , PARTCODE FROM SOM_TABS_CODES WHERE CODENAME = \"AUTO_REPEAT_RATE\"  and LANGUAGE_NAME = '" + this.userLang + "' order by CODETEXT_LANG+0 ",
			"lkpArrName":"lkpArrAUTO_REPEAT_RATE"},
	{"statment":"SELECT CODE, CODETEXT_LANG , PARTCODE FROM SOM_TABS_CODES WHERE CODENAME = \"AUTO_REPEAT_DELAY\"  and LANGUAGE_NAME = '" + this.userLang + "' order by CODETEXT_LANG ",
			"lkpArrName":"lkpArrAUTO_REPEAT_DELAY"}];
...
public lkpArrAUTO_REPEAT_RATE = [];

public lkpArrAUTO_REPEAT_DELAY = [];
...
public lkpArrGetAUTO_REPEAT_RATE(CODE: any): any {
var rec = this.lkpArrAUTO_REPEAT_RATE.find((x:any) => x.CODE === CODE);
return rec;
}

public lkpArrGetAUTO_REPEAT_DELAY(CODE: any): any {
var rec = this.lkpArrAUTO_REPEAT_DELAY.find((x:any) => x.CODE === CODE);
return rec;
}

onChanges(): void {
this.form.get('ARROW_BUTTON_TIMING_ID').valueChanges.subscribe(val => {
});
}
...
public printScreen(){
  ...
  public handleComponentConfig(ComponentConfig:any) {
    if (typeof ComponentConfig !== "undefined") {
      if (this.paramConfig.DEBUG_FLAG) console.log("ScdArrowButtonTimingScdAbtArrowButtonTimingForm ComponentConfig:", {...ComponentConfig});
...
async WHEN_VALIDATE_ITEM_ARROW_BUTTON_TIMING_ID(value) {

 this.FORM_TRIGGER_FAILURE = false ; 
 if (typeof this.form.controls['ARROW_BUTTON_TIMING_ID'] != "undefined" ) 
      this.form.controls['ARROW_BUTTON_TIMING_ID'].setErrors({invalid: true}); 
 // Code goes here 
 ...
 if (typeof this.form.controls['ARROW_BUTTON_TIMING_ID'] != "undefined" ) 
     this.form.get('ARROW_BUTTON_TIMING_ID').updateValueAndValidity();
 this.form.updateValueAndValidity(); 
 }

 async ON_CLICK_ARROW_BUTTON_TIMING_ID(event){
...
async WHEN_VALIDATE_ITEM_AUTO_REPEAT_RATE(value) {

 this.FORM_TRIGGER_FAILURE = false ; 
 if (typeof this.form.controls['AUTO_REPEAT_RATE'] != "undefined" ) 
      this.form.controls['AUTO_REPEAT_RATE'].setErrors({invalid: true}); 
 // Code goes here 
 ...
 if (typeof this.form.controls['AUTO_REPEAT_RATE'] != "undefined" ) 
     this.form.get('AUTO_REPEAT_RATE').updateValueAndValidity();
 this.form.updateValueAndValidity(); 
 }

 async ON_CLICK_AUTO_REPEAT_RATE(event){

}

async WHEN_VALIDATE_ITEM_AUTO_REPEAT_DELAY(value) {

 this.FORM_TRIGGER_FAILURE = false ; 
 if (typeof this.form.controls['AUTO_REPEAT_DELAY'] != "undefined" ) 
      this.form.controls['AUTO_REPEAT_DELAY'].setErrors({invalid: true}); 
 // Code goes here 
 ...
 if (typeof this.form.controls['AUTO_REPEAT_DELAY'] != "undefined" ) 
     this.form.get('AUTO_REPEAT_DELAY').updateValueAndValidity();
 this.form.updateValueAndValidity(); 
 }

 async ON_CLICK_AUTO_REPEAT_DELAY(event){

}
 
 async onChange_ARROW_BUTTON_TIMING_ID(event:any) { 
 var value = event.target.value; 
 if ((value == null) || (value == '')) 	
 	return;  
    this.FORM_TRIGGER_FAILURE = false;	
 await   this.WHEN_VALIDATE_ITEM_ARROW_BUTTON_TIMING_ID(value); if ( this.FORM_TRIGGER_FAILURE) return; 
 this.formValidationChangedOutput.emit(this.form.valid); 
 ...
 async onValueChange_AUTO_REPEAT_RATE(value) { 
  this.FORM_TRIGGER_FAILURE = false;	
 await this.WHEN_VALIDATE_ITEM_AUTO_REPEAT_RATE(value); if ( this.FORM_TRIGGER_FAILURE) return; 
 this.formValidationChangedOutput.emit(this.form.valid); 
  
  } 
 async onValueChange_AUTO_REPEAT_DELAY(value) { 
  this.FORM_TRIGGER_FAILURE = false;	
 await this.WHEN_VALIDATE_ITEM_AUTO_REPEAT_DELAY(value); if ( this.FORM_TRIGGER_FAILURE) return; 
 this.formValidationChangedOutput.emit(this.form.valid); 
