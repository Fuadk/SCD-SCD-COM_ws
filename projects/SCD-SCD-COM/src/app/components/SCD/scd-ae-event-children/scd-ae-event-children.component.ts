...
import {   scdeventChildScdAeEventChildren , componentConfigDef } from '@modeldir/model';
...
'EVENT_CHILD_ID' : new FormControl(dataItem.EVENT_CHILD_ID  , ) ,
'EVENT_ID' : new FormControl(dataItem.EVENT_ID  ,   Validators.required ) ,
...
@Component({
  selector: 'app-scd-ae-event-children',
  encapsulation: ViewEncapsulation.None,
  templateUrl: './scd-ae-event-children.component.html',
  standalone: false,
  styleUrls: ['./scd-ae-event-children.component.scss'
],
...
export class ScdEventChildScdAeEventChildrenGridComponent implements OnInit,OnDestroy {
 ...
 public showToolBar = true;
  ...
  public isMaster: boolean = false;
  		...
  		public  isEVENT_CHILD_IDEnable : boolean = true; 
public  isEVENT_IDEnable : boolean = true; 
  ...
  private masterKey ="";
  private masterKeyName ="EVENT_ID";
  private insertCMD = "INSERT_SCD_EVENT_CHILD";
  private updateCMD = "UPDATE_SCD_EVENT_CHILD";
  private deleteCMD =   "DELETE_SCD_EVENT_CHILD";
  private getCMD = "GET_SCD_EVENT_CHILD_QUERY";

  public  executeQueryresult:any;
  public title =  this.starServices.getNLS([],"SCD_AE_EVENT_CHILDREN.scdeventChildScdAeEventChildren.component_title","Event Children");
  public PDFfileName = this.title + ".PDF";
  public ExcelfileName = this.title + ".xlsx";
  public componentConfig
  public compTitleMsg =  "SCD_AE_EVENT_CHILDREN.scdeventChildScdAeEventChildren";
  ...
  public primarKeyReadOnlyArr = {isEVENT_CHILD_IDreadOnly : false , isEVENT_IDreadOnly : false};  
  public paramConfig;
...
public visibleEVENT_CHILD_ID = false;
...
public compSelector = 'app-scd-ae-event-children';
  ...
  @Output() formValidationChangedOutput: EventEmitter<boolean> = new EventEmitter();
    ...
    constructor(public router: Router,public intl: IntlService, public responsive: BreakpointObserver, private starNotify: StarNotifyService,   public starServices: starServices, private renderer: Renderer2) {
      ...
      this.title = this.componentConfig.title ? this.componentConfig.title :this.starServices.getNLS([],"SCD_AE_EVENT_CHILDREN.scdeventChildScdAeEventChildren.component_title","Event Children");
  }
...
//Next part for filtering
  ...
  @Input() public set detail_Input(grid: any) {
    ...
    /*
    if ( (grid.EVENT_ID != "") &&   (typeof grid.EVENT_ID != "undefined"))
    {
      this.masterKey = grid.EVENT_ID;
  ...
  private gridInitialValues:any = new scdeventChildScdAeEventChildren();   
 ...
 async cellClickHandler(event:any ) {
  ...
  if (typeof this[name] === 'function') {
    ...
    this.readCompletedOutput.emit(this.formGroup.value);
     ...
     let componentConfig = new componentConfigDef();
      ...
      let masterKeyArr = [this.formGroup.value['EVENT_ID']];
      let masterKeyNameArr = ['EVENT_ID'];
  ...
  public enterQuery (grid : GridComponent): void{
   ...
   async callBackFunction(data:any) {
      ...
      let masterKeyArr = [GridData.data[0]['EVENT_ID']];
      let masterKeyNameArr = ['EVENT_ID'];
      //for (let i = 0; i < masterKeyNameArr.length; i++) {
      //  componentConfig.[masterKeyNameArr[i]] = masterKeyArr[i];
      //}
  ...
  public commonCallStarNotify(data:any) {
    ...
    let masterParams = {
      data: this.masterKeyArr
       ...
       let masterKeyArr = [data['EVENT_ID'],data['EVENT_CHILD_ID']];
      let masterKeyNameArr = ['EVENT_ID','EVENT_CHILD_ID'];
      //for (let i = 0; i < masterKeyNameArr.length; i++) {
      //  componentConfig.[masterKeyNameArr[i]] = masterKeyArr[i];
      //}
...
this.lookupArrDef =[	{"statment":"SELECT EVENT_ID CODE, ACTION_NAME CODETEXT_LANG  FROM SCD_EVENT  order by CODETEXT_LANG ",
			"lkpArrName":"lkpArrEVENT_ID"}];
 if (this.lookupArrDef.length > 0)
   this.starServices.fetchLookups(this, this.lookupArrDef);
}

public lkpArrEVENT_ID = [];

public lkpArrGetEVENT_ID(CODE: any): any {
var rec = this.lkpArrEVENT_ID.find((x:any) => x.CODE === CODE);
return rec;
}
  ...
  window.print();
   ...
   public handleComponentConfig(ComponentConfig:any) {
      ...
      if (typeof ComponentConfig !== "undefined") {
         if (this.paramConfig.DEBUG_FLAG) console.log("ScdEventChildScdAeEventChildrenGrid ComponentConfig:", ComponentConfig);
...
async WHEN_VALIDATE_ITEM_EVENT_CHILD_ID(formGroup) {

 this.FORM_TRIGGER_FAILURE = false ; 
 if (typeof this.formGroup.controls['EVENT_CHILD_ID'] != "undefined" ) 
      this.formGroup.controls['EVENT_CHILD_ID'].setErrors({invalid: true}); 
 // Code goes here 
 ...
 if (typeof this.formGroup.controls['EVENT_CHILD_ID'] != "undefined" ) 
     this.formGroup.get('EVENT_CHILD_ID').updateValueAndValidity();
 this.formGroup.updateValueAndValidity(); 
 }

 async ON_CLICK_EVENT_CHILD_ID(event){
...
async WHEN_VALIDATE_ITEM_ENABLED(formGroup) {

 this.FORM_TRIGGER_FAILURE = false ; 
 if (typeof this.formGroup.controls['ENABLED'] != "undefined" ) 
      this.formGroup.controls['ENABLED'].setErrors({invalid: true}); 
 // Code goes here 
 ...
 if (typeof this.formGroup.controls['ENABLED'] != "undefined" ) 
     this.formGroup.get('ENABLED').updateValueAndValidity();
 this.formGroup.updateValueAndValidity(); 
 }
...
async WHEN_VALIDATE_ITEM_ACTION_NAME(formGroup) {

 this.FORM_TRIGGER_FAILURE = false ; 
 if (typeof this.formGroup.controls['ACTION_NAME'] != "undefined" ) 
      this.formGroup.controls['ACTION_NAME'].setErrors({invalid: true}); 
 // Code goes here 
 ...
 if (typeof this.formGroup.controls['ACTION_NAME'] != "undefined" ) 
     this.formGroup.get('ACTION_NAME').updateValueAndValidity();
 this.formGroup.updateValueAndValidity(); 
 }
...
async WHEN_VALIDATE_ITEM_DESCRIPTION(formGroup) {
 ...
 async onBlur_EVENT_CHILD_ID() { 
  await this.WHEN_VALIDATE_ITEM_EVENT_CHILD_ID(this.formGroup); if ( this.FORM_TRIGGER_FAILURE) return;  
 } 
 async valueChangeEVENT_ID(value: any) { 
 await this.WHEN_VALIDATE_ITEM_EVENT_ID(this.formGroup); if ( this.FORM_TRIGGER_FAILURE) return;  
 } 
 async onBlur_ENABLED() { 
  await this.WHEN_VALIDATE_ITEM_ENABLED(this.formGroup); if ( this.FORM_TRIGGER_FAILURE) return;  
 } 
 async onBlur_ACTION_NAME() { 
  await this.WHEN_VALIDATE_ITEM_ACTION_NAME(this.formGroup); if ( this.FORM_TRIGGER_FAILURE) return;  
 } 
