...
async onCloseWindow(windowId: string): Promise<void> {
  ...
  const hasChanges = await this.windowManager.checkWindowHasUnsavedChanges(windowId);
  console.log('onCloseWindowDebug:windowId:', windowId, 'hasChanges:', hasChanges);
  
  if (hasChanges) {
    // Show confirmation dialog
    this.showCloseConfirmationDialog(windowId);
  } else {
    // Close immediately
    this.windowManager.closeWindow(windowId);
  }
}
private showCloseConfirmationDialog(windowId: string): void {
  console.log('showCloseConfirmationDialog: Opening dialog for window:', windowId);
  ...
  dialog.result.subscribe((result: any) => {
    console.log('showCloseConfirmationDialog: Dialog result:', result);
    ...
    if (result && result.text === 'Save') {
      ...
      console.log('showCloseConfirmationDialog: User cancelled or closed dialog');
    }
  });
}
private async handleSaveAction(windowId: string): Promise<void> {
    console.log('handleSaveAction: Starting save for window:', windowId);

    try {
      // Get the window from the windows array
      const window = this.windows.find(w => w.id === windowId);

      if (!window) {
        console.error('handleSaveAction: Window not found:', windowId);
        return;
      }

      // Get the component instance stored in the window
      const componentInstance = window.componentInstance;

      if (!componentInstance) {
        console.error('handleSaveAction: No component instance for window:', windowId);
        return;
      }

      console.log('handleSaveAction: Sending masterSaved to component:', componentInstance.constructor?.name);
      ...
      console.log('handleSaveAction: masterSaved sent to component');

      // Wait a moment for the save to complete
      // The component will emit isDirty = false via componentConfigChanged when done
      await this.starServices.sleep(500);

      // Close the window after save
      console.log('handleSaveAction: Closing window');
      this.windowManager.closeWindow(windowId);
...
async saveWindowChanges(windowId: string): Promise<void> {
  ...
  // Update the componentConfig to trigger save
  if (window.inputs && window.inputs.setComponentConfig_Input) {
    window.inputs.setComponentConfig_Input.masterSaved = true;
...
onComponentConfig_Output(event: { windowId: string, componentConfig: any }): void {
  console.log('onCloseWindowDebug:onComponentConfig_Output called:', event);
  if (event && event.componentConfig) {
    // Update the window's dirty state in the service
    this.windowManager.updateWindowDirtyState(
      event.windowId, 
