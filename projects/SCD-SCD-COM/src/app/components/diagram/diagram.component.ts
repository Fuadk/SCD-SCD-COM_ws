...
export class DiagramComponent implements AfterContentInit, OnChanges, OnDestroy, OnInit {
  ...
  public initDiagram() {
    console.log("initDiagram:here1")
    if (this.useModeler)
      this.bpmnJS = this.bpmnJSM;
    else
      this.bpmnJS = this.bpmnJSV;
  console.log("initDiagram:here2")
    this.bpmnJS.on<ImportDoneEvent>('import.done', ({ error }) => {
      console.log("initDiagram:here3")
      if (!error) {
        //this.bpmnJS.get<Canvas>('canvas').zoom('fit-viewport');
        this.bpmnJS.get<Canvas>('canvas');
        this.eventBus = this.bpmnJS.get('eventBus');
        // console.log ('eventBus:', this.eventBus);
        this.interaction(this, this.eventBus);
      }
    });
    console.log("initDiagram:here4")
    this.bpmnJS.attachTo(this.el.nativeElement);
    console.log("initDiagram:here5")
  }
  /**
  ...
  public handleEvent(masterparam) {
    ...
    if (type == "bpmn:Task") {
      ...
      while (i < this.workOrders.length) {
        if (id == this.workOrders[i].SEQUENCE_NAME) {
      ...
      // let i=0;
      // while (i<this.workOrders.length){
      //   if (ruleID == this.workOrders[i].SEQUENCE_NAME){
      //     this.showWODetails(this.workOrders[i]);
      //     break;
      //   }
      //   i++;
  ...
  public deltaYPos =100;
  public deltaXPos =100;
  ...
  + '<bpmndi:BPMNPlane id="BPMNPlane_1" bpmnElement="Process_1">'
  + '<bpmndi:BPMNShape id="_BPMNShape_StartEvent_2" bpmnElement="StartEvent_1">'
  + '<dc:Bounds x="' + this.startXPos + '" y="' + this.startYPos + '" width="36" height="36" />'
  + '<bpmndi:BPMNLabel>'
  ...
  + '</bpmndi:BPMNDiagram>'
  + '</bpmn2:definitions>';
  public addRuleEvent(SEQUENCE_NAME, rulesDef, taskLast, xPos, yPos, process, workOrders, Orders) {
    let i = 0;
    // let rulesFound =[];
    let rulesKeyFound = [];
    let retVal = {
    ...
    console.log("searching for SEQUENCE_NAME :", SEQUENCE_NAME, " in rulesDef:", rulesDef)
    while (i < rulesDef.length) {
      if (rulesDef[i].SEQUENCE_NAME == SEQUENCE_NAME) {
        //rulesFound.push(rulesDef[i].RULE_ID);
        let id = "Event_" + SEQUENCE_NAME + "_" + rulesDef[i].RULE_ID;
        let name = 'Rule:' + rulesDef[i].RULE_ID + ' ' + rulesDef[i].RULE_DESCRIPTION;
        //name ="";
        console.log("id:", id, "name:", name, " rulesDef[i].RULE_ID:", rulesDef[i].RULE_ID)
        ...
        //if (rulesDef[i].SEQUENCE_NAME == "")
        //  yPos = yPos + this.deltaYPos;
        this.modeling.createShape(taskx, { x: xPos, y: yPos+ this.deltaYPos }, process);
  ...
  public adjustConnections(){
    //https://forum.bpmn.io/t/create-or-align-waypoints-automatically/5787/4
    const connections = this.elementRegistry.filter((element) => element.waypoints);

    connections.forEach((connection) => {
      this.modeling.updateWaypoints(connection, [
        getMid(connection.source),
        getMid(connection.target)
      ]);

      this.modeling.layoutConnection(connection, {
  ...
  public endAdded = false;
  public endID;

  public showEnd(process ,workOrders, i, xPos,yPos, taskLast ){
    let connection;
    if (!this.endAdded){
      let id = "End"
      let name = 'End';
      //name ="";
      let eventBusinessObjectx = this.bpmnFactory.create('bpmn:EndEvent', {
        id: id, name: name
      });
      let EndEvent = this.elementFactory.createShape({ type: 'bpmn:EndEvent', businessObject: eventBusinessObjectx });
      this.modeling.createShape(EndEvent, { x: xPos, y: yPos }, process);
      connection = this.modeling.createConnection(taskLast, EndEvent, { type: 'bpmn:SequenceFlow' }, process);
      this.endAdded = true;
      this.endID  = EndEvent;
  
    }
    else{
      connection = this.modeling.createConnection(taskLast, this.endID, { type: 'bpmn:SequenceFlow' }, process);
    }
    if ( i != -1){
      if (workOrders[i].APPROVE_SEQ == -1) {
        this.modeling.updateLabel(connection, 'Approved');
      }
        else if (workOrders[i].REJECT_SEQ == -1){
          this.modeling.updateLabel(connection, 'Rejected');
      }
    }
  }
  public showPath(workOrders, rulesDef, Orders, rejectPath, startEvent, process, EndEvent, retVal) {
    function getNextI(i, SEQ){
      let NextI = -1;
      let j = 0;
      while ( j < workOrders.length) {
        if (workOrders[i][SEQ] == workOrders[j].SEQUENCE_NAME){
          NextI = j;
          break;
        }
        j++;
    ...
    let xPos = this.deltaXPos * 1.5;
    let yPos = this.deltaYPos/2;
    if (rejectPath == true)
      yPos = this.deltaYPos * 1.5;
    let taskLast;
    let SEQUENCE_NAME = 0;
    taskLast = startEvent;
    console.log("showPath:",retVal);
    let i = retVal.NextI;
    let aprvRject = "";
    if (retVal.NextI > 0 ){
      //taskLast = this.elementRegistry.get(retVal.lastID);
      taskLast = retVal.lastID;
      console.log("rejectFlag:rejectPath:taskLast:",rejectPath, taskLast);
      
      xPos = taskLast.x + this.deltaXPos/2;
      yPos = taskLast.y + (this.deltaYPos *1.5);
      aprvRject = retVal.aprvRject;
      
    }
    retVal.NextI = -1;
    console.log("showPath:aprvRject:", aprvRject);
    
   
    while ( i < workOrders.length) {
      
      //console.log("rejectFlag:rejectPath:workOrders:",rejectPath, workOrders[i]);

      let name = workOrders[i].taskName;
      let id = workOrders[i].taskId;
      ...
      let connection = this.modeling.createConnection(taskLast, taskx, { type: 'bpmn:SequenceFlow' }, process);
      if (aprvRject != ""){
        this.modeling.updateLabel(connection, aprvRject);
        aprvRject = "";
      }
      taskLast = taskx;


      this.addRuleEvent(workOrders[i].SEQUENCE_NAME, rulesDef, taskLast, xPos, yPos, process, workOrders, Orders);
      xPos = xPos + (this.deltaXPos * 1.5);
      let NextI = -1;
      retVal = {
        lastID :taskx,
        NextI: NextI,
      ...
      if ( (workOrders[i].APPROVE_SEQ == -1) ||  (workOrders[i].REJECT_SEQ == -1) )
        {
          this.showEnd(process, workOrders, i, xPos,yPos, taskLast);
          
          if (workOrders[i].APPROVE_SEQ == -1) {
            NextI = getNextI(i, 'REJECT_SEQ');
            aprvRject = "Rejected";
            ...
            if (workOrders[i].APPROVE_SEQ != ""){
              NextI = getNextI(i, 'APPROVE_SEQ');
            }
            else{
              NextI = i + 1;
            }
          ...
          yPos = yPos + (this.deltaYPos * 1.5);
        //break;
      }
   
      // let taskApprvd = this.elementRegistry.get(id);
      // console.log("idInfo:", taskLast, taskApprvd);
      // if (typeof taskApprvd !== "undefined") {
      //   this.modeling.createConnection(taskLast, taskApprvd, { type: 'bpmn:SequenceFlow'
      //     , waypoints: [ 
      //       { x: taskLast.x + taskLast.width,         y: taskLast.y + (taskLast.height/2) }, 
      //       { x: taskApprvd.x + (taskApprvd.width/2), y: taskLast.y + (taskLast.height/2) },
      //       { x: taskApprvd.x + (taskApprvd.width/2), y: taskApprvd.y + taskApprvd.height }  ] 
      //   }, process);
      // }
  ...
  async buildWorkOrdersBO(workOrders, rulesDef, Orders) {
    ...
    if (workOrders.length != 0 ){
      ...
      let yPos = this.deltaYPos/2;
      let xPos = this.deltaXPos/2;
      let taskLast;
      let SEQUENCE_NAME = 0;
      taskLast = startEvent;

      this.addRuleEvent(SEQUENCE_NAME, rulesDef, taskLast, xPos, yPos, process, workOrders, Orders);
      xPos = this.deltaXPos * 1.5;
      ...
      while (retVal.NextI != -1 )
        retVal = this.showPath(workOrders, rulesDef, Orders, rejectPath, startEvent, process, null, retVal);
      console.log("retVal:", retVal) 
      this.showEnd(process, workOrders, -1, retVal.xPos, retVal.yPos, retVal.taskLast);
  ...
  async viewWorkOrders(xml, workOrders) {
    ...
    for (let i = 0; i < workOrders.length; i++) {
      let id = "Task" + workOrders[i].SEQUENCE_NAME;
      let status = workOrders[i].WO_STATUS;
      if (typeof status !== "undefined") {
        let htmlTtxt = '<div></div>';
        htmlTtxt = workOrders[i].htmlTtxt;
   ...
   public hideLogo()
 {
     ...
     for (let i=0;i<collection.length;i++){
       ...
       //console.log ("checking:collection[i].attributes:",collection[i].attributes[0]);
       //console.log ("checking:innerHTML:",innerHTML);
       let result = innerHTML.includes("http://www.w3.org/2000/svg");
  ...
  public showRuleDetails(ruleID, workOrders, rulesDef, Orders) {
    ...
    this.ADM_RULESConfig.showHideGridFlag = true
  ...
  public showWODetails(dataItem) {
    ...
    this.DSP_WORK_ORDERFormConfig.enabled = false;
