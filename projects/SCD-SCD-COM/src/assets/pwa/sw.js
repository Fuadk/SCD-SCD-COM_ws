...
const CACHE_NAME = 'MyApps-app-v2';
...
const ASSETS_TO_CACHE = [
  ...
  '/SCD-SCD-COM/styles/main.css',
  '/SCD-SCD-COM/scripts/app.js'
];
...
self.addEventListener('push', event => {
  ...
  const options = {
    body: event.data ? event.data.text() : 'New update in MyApps system',
  ...
  event.waitUntil(
    self.registration.showNotification('MyApps System', options)
  );
});
...
self.addEventListener('notificationclick', event => {
  ...
  if (event.action === 'open') {
    event.waitUntil(
      clients.openWindow(event.notification.data.url || '/SCD-SCD-COM/')
    );
  }
});
...
async function syncPendingForms() {
  ...
  for (const form of pendingForms) {
    try {
      const response = await fetch('/SCD-SCD-COM/api/sync', {
        method: 'POST',
        headers: {
